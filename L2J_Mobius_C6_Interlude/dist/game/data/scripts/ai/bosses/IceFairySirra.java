	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			case "check_condition":
			{
				...
				if (player.isInParty() && (player.getParty().getPartyLeaderOID() == player.getObjectId()))
				{
					if (checkItems(player))
					{
						...
						npc.setBusy(true);
						screenMessage(player, "Steward: Por favor, espere um momento.", 100000);
			...
			case "Party_Port":
			{
				...
				screenMessage(player, "Steward: Por favor, restaure a aparência da Rainha!", 10000);
				startQuestTimer("30MinutesRemaining", 300000, null, player);
				break;
			}
			case "30MinutesRemaining":
			{
				screenMessage(player, "Restam 30 minuto(s).", 10000);
				startQuestTimer("20minutesremaining", 600000, null, player);
				break;
			}
			case "20MinutesRemaining":
			{
				screenMessage(player, "Restam 20 minuto(s).", 10000);
				startQuestTimer("10minutesremaining", 600000, null, player);
				break;
			}
			case "10MinutesRemaining":
			{
				screenMessage(player, "Steward: Não perca tempo! Por favor, despacha-te!", 10000);
				break;
			}
			case "End":
			{
				screenMessage(player, "Steward: Foi realmente pedir demais.", 10000);
				cleanUp();
				break;
			}
		}
