...
public class Core extends Quest
{
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet)
	{
		if (npc.getNpcId() == CORE)
		{
			if (_firstAttacked)
			{
				if (getRandom(100) == 0)
				{
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, npc.getName(), "Removendo intrusos."));
				}
			}
			else
			{
				_firstAttacked = true;
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, npc.getName(), "Um alvo não permitido foi descoberto."));
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, npc.getName(), "Iniciando o sistema de remoção de intrusos."));
			}
		}
	...
	public String onKill(Npc npc, Player killer, boolean isPet)
	{
		...
		if (npcId == CORE)
		{
			...
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, name, "Ocorreu um erro fatal."));
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, name, "O sistema está sendo desligado..."));
