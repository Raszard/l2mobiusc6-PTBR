 ...
 * @author JOJO Updated by rocknow, L2jOff team, L2jFrozen team.
 */
public class Frintezza extends Quest
{
	...
	private static final int[][] MONSTER_LOC =
	{
	...
	private GrandBoss _weakScarlet;
	private GrandBoss _strongScarlet;
	private Monster _demon1;
	private Monster _demon2;
	private Monster _demon3;
	private Monster _demon4;
	private Monster _portrait1;
	private Monster _portrait2;
	private Monster _portrait3;
	private Monster _portrait4;
	...
	private final Collection<Player> _playersInside = ConcurrentHashMap.newKeySet();
	private final Collection<Npc> _room1Monsters = ConcurrentHashMap.newKeySet();
	private final Collection<Npc> _room2Monsters = ConcurrentHashMap.newKeySet();
	...
	public Frintezza()
	{
		...
		registerMobs(SCARLET1, SCARLET2, FRINTEZZA, 18328, 18329, 18330, 18331, 18332, 18333, 18334, 18335, 18336, 18337, 18338, 18339, 29048, 29049, 29050, 29051);
		addStartNpc(GUIDE);
		addTalkId(GUIDE);
		addStartNpc(CUBE);
		addTalkId(CUBE);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "room1_spawn":
			{
				for (int i = 0; i <= 17; i++)
				{
					final Npc mob = addSpawn(MONSTER_LOC[i][0], MONSTER_LOC[i][1], MONSTER_LOC[i][2], MONSTER_LOC[i][3], MONSTER_LOC[i][4], false, 0);
			...
			case "camera_18b":
			{
				...
				_weakScarlet = (GrandBoss) addSpawn(SCARLET1, 174232, -88020, -5110, 16384, false, 0);
			...
			case "morph_06":
			{
				...
				_weakScarlet.deleteMe();
				_weakScarlet = null;
				_weakScarlet = (GrandBoss) addSpawn(SCARLET1, _scarletX, _scarletY, _scarletZ, _scarletHeading, false, 0);
				_weakScarlet.setInvul(true);
				_weakScarlet.setImmobilized(true);
				_weakScarlet.disableAllSkills();
				_weakScarlet.setRHandId(7903);
			...
			case "morph_13":
			{
				_strongScarlet = (GrandBoss) addSpawn(SCARLET2, _scarletX, _scarletY, _scarletZ, _scarletHeading, false, 0);
			...
			case "float_effect":
			{
				...
				else
				{
					for (Creature creature : _zone.getCharactersInside())
					{
						if ((creature instanceof Player) && (creature.getFirstEffect(5016) != null))
						{
							creature.abortAttack();
							creature.abortCast();
							creature.disableAllSkills();
			...
			case "room_final":
			{
				_zone.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.SHOUT, npc.getName(), "Excedeu o limite de tempo, o desafio falhou!"));
			...
			case "spawn_minion":
			{
				...
				{
					final Npc mob = addSpawn(npc.getNpcId() + 2, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false, 0);
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		switch (status)
		{
			case DEAD:
			{
				htmltext = "<html><body>Não há nada além do Campo de Força Mágica. Volte mais tarde.<br>(Você não pode entrar porque Frintezza não está dentro da Tumba Imperial.)</body></html>";
				break;
			}
			case DORMANT:
			{
				...
				if (!Config.BYPASS_FRINTEZZA_PARTIES_CHECK)
				{
					...
					{
						htmltext = "<html><body>Nenhuma reação. O contato deve ser iniciado pelo Líder do Canal de Comando.</body></html>";
					...
					{
						htmltext = "<html><body>Seu canal de comando precisa ter pelo menos " + Config.FRINTEZZA_MIN_PARTIES + " partes e no máximo " + Config.FRINTEZZA_MAX_PARTIES + ".</body></html>";
						partyCheckSuccess = false;
					}
				}
				if (partyCheckSuccess)
				{
					...
					{
						htmltext = "<html><body>Você não possui o item necessário.</body></html>";
					}
					else
					{
									...
									for (Party party : cc.getParties())
									{
										...
										synchronized (_playersInside)
										{
											for (Player member : party.getPartyMembers())
											{
												...
												{
													member.sendMessage("O número de desafios está cheio, então não posso entrar.");
													break;
												}
			...
			default:
			{
				htmltext = "<html><body>Alguém já está dentro do Campo de Força Mágica. Tente mais tarde.</body></html>";
				break;
			}
		}
