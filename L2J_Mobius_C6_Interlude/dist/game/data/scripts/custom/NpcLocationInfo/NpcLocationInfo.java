...
public class NpcLocationInfo extends Quest
{
	private static final int[] RADARS =
	{
		...
		30033, // Magistrado Baulro
		30035, // Magistrado Harrys
		...
		30157, // Magistrado Greenis
		30158, // Magistrado Esrandell
		...
		if (Util.isDigit(event))
		{
			...
			if (Util.contains(RADARS, npcId))
			{
				for (Spawn spawn : SpawnTable.getInstance().getSpawnTable().values())
				{
					...
					{
						st.clearRadar();
						st.addRadar(spawn.getX(), spawn.getY(), spawn.getZ());
						break;
					}
				}
