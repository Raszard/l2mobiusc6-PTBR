...
public class Q421_LittleWingsBigAdventure extends Quest
{
	...
	{
		super(421, "A Grande Aventura da Pequena Asa");
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet)
	{
		...
		if (((Monster) npc).hasMinions())
		{
			for (Monster ghost : ((Monster) npc).getSpawnedMinions())
			{
				...
				{
					ghost.broadcastNpcSay("Temos que proteger a árvore das fadas!");
				}
			}
		}
		...
		if (isPet && (attacker.getPet().getControlItemId() == st.getInt("summonOid")))
		{
			...
			if ((getRandom(100) < 1) && st.hasQuestItems(FAIRY_LEAF))
			{
				...
				if ((iCond | idMask) != iCond)
				{
					...
					npc.broadcastNpcSay("Dê-me uma Folha de Fada...!");
					st.takeItems(FAIRY_LEAF, 1);
					npc.broadcastNpcSay("Vá embora agora, antes que você incorra na ira do fantasma guardião...");
