 ...
 */
package quests.Q229_TestOfWitchcraft;
		...
		switch (npc.getNpcId())
		{
			case NAMELESS_REVENANT:
			{
				if (st.hasQuestItems(LARA_MEMO) && !npc.isScriptValue(1))
				{
					...
					npc.broadcastNpcSay("Eu não posso dar isso a você de jeito nenhum! É minha joia preciosa!");
				}
				break;
			}
			...
			case DREVANUL_PRINCE_ZERUEL:
			{
				if (st.isCond(4) && !npc.isScriptValue(1))
				{
					...
					npc.setScriptValue(1);
					npc.broadcastNpcSay("Eu vou tirar suas vidas mais tarde!!");
				...
				else if (st.isCond(9) && _drevanulPrinceZeruel)
				{
					if (attacker.getInventory().getPaperdollItemId(7) == SWORD_OF_BINDING)
					{
						...
						{
							npc.setScriptValue(1);
							npc.broadcastNpcSay("Essa espada é realmente...!");
						}
					}
					else
					{
