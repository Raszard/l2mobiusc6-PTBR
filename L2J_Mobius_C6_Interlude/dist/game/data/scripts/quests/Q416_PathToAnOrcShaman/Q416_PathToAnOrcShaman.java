...
public class Q416_PathToAnOrcShaman extends Quest
{
	...
	{
		super(416, "Caminho para um Orc Shaman");
			...
			case BLACK_LEOPARD:
			{
				if (st.isCond(14))
				{
					if (st.getInt("leopard") > 0)
					{
						...
						if (getRandom(3) < 2)
						{
							npc.broadcastNpcSay("Meu querido amigo " + player.getName() + ", que foi na minha frente!");
						}
					}
					else
					{
				...
				else if (st.isCond(16))
				{
					...
					if (getRandom(3) < 2)
					{
						npc.broadcastNpcSay("Ouça Tejakar Gandi, jovem Oroka! O espírito do leopardo morto está chamando você, " + player.getName() + "!");
					}
				}
