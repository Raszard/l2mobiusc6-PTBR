...
public class SagasSuperClass extends Quest
{
	...
	public QuestState findRightState(Npc npc)
	{
		...
		if (SPAWN_LIST.containsKey(npc))
		{
			player = (Player) World.getInstance().findObject(SPAWN_LIST.get(npc));
			if (player != null)
			{
	...
	public void giveHallishaMark(QuestState st2)
	{
		if (st2.getInt("spawned") == 0)
		{
			if (st2.getQuestItemsCount(_items[3]) >= 700)
			{
				...
				final Npc archon = st2.addSpawn(_mob[1], xx, yy, zz);
				addSpawn(st2, archon);
				st2.set("spawned", "1");
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (st != null)
		{
			...
			else if (event.equals("7-1"))
			{
				...
				{
					final Npc mob1 = st.addSpawn(_mob[0], _x[0], _y[0], _z[0]);
					st.set("spawned", "1");
			...
			else if (event.equals("10-1"))
			{
				...
				{
					final Npc mob3 = st.addSpawn(_mob[2], _x[1], _y[1], _z[1]);
					final Npc mob2 = st.addSpawn(_npc[4], _x[2], _y[2], _z[2]);
					addSpawn(st, mob3);
					addSpawn(st, mob2);
	...
	{
		String htmltext = "<html><body>Você não está em uma missão que envolve este NPC ou não atende aos requisitos mínimos da missão deste NPC.</body></html>";
		...
		if (st != null)
		{
			...
			{
				htmltext = "<html><body>Você já completou esta missão!</body></html>";
			}
