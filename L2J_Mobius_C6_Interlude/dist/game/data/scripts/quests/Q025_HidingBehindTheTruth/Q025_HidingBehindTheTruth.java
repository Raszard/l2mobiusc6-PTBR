...
public class Q025_HidingBehindTheTruth extends Quest
{
	...
	{
		super(25, "Escondido Atrás da Verdade");
		...
		switch (event)
		{
			...
			case "31535-03.htm":
			{
				if (qs.getInt("step") == 0)
				{
					...
					triol.broadcastPacket(new CreatureSay(triol.getObjectId(), ChatType.GENERAL, triol.getName(), "Essa caixa foi selada pelo meu mestre. Não toque nela!"));
					triol.setRunning();
	...
	public String onKill(Npc npc, Player player, boolean isPet)
	{
		...
		if ((qs.getState() == State.STARTED) && qs.isCond(7))
		{
			...
			qs.setCond(8);
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), ChatType.GENERAL, npc.getName(), "Você acabou com minha vida imortal! Você foi protegido pelo senhor feudal, não é?"));
