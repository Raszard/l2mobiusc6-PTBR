...
public class Q335_TheSongOfTheHunter extends Quest
{
	...
	public String onKill(Npc npc, Player player, boolean isPet)
	{
			...
			if (request != null)
			{
				...
				if (request.spawnlist.containsKey(npcId) && (st.getQuestItemsCount(request.request_item) < request.request_count))
				{
					...
					if (getRandom(100) < spawnChance[1])
					{
						...
						{
							npc.broadcastNpcSay("Mostre-me as coisas bonitas e brilhantes! Elas são todas minhas!");
						}
					}
				}
			}
		...
		{
			npc.broadcastNpcSay("Tomaremos a propriedade do antigo império!");
