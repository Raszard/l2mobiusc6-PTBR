	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isPet)
	{
		...
		switch (npc.getNpcId())
		{
			case ARK_GUARDIAN_ELBEROTH:
			{
				...
				{
					npc.broadcastNpcSay("...Sinto muito, mas tirei sua vida.");
			...
			case ARK_GUARDIAN_SHADOW_FANG:
			{
				if (npc.isScriptValue(0))
				{
					npc.broadcastNpcSay("Cobrirei esta montanha com seu sangue!");
					npc.setScriptValue(1);
				}
				break;
			}
			case ANGEL_KILLER:
			{
				if (npc.isScriptValue(0))
				{
					npc.broadcastNpcSay("Haha.. Realmente divertido! Quanto à chave, procure no cadáver!");
					npc.setScriptValue(1);
				}
				...
				{
					npc.abortAttack();
					npc.broadcastNpcSay("Não consigo me livrar de você... Você pegou a chave do cadáver?");
					npc.decayMe();
		...
		switch (npc.getNpcId())
		{
			...
			case ARK_GUARDIAN_ELBEROTH:
			{
				if ((cond >= 5) && (cond <= 22) && !st.hasQuestItems(SECOND_KEY_OF_ARK))
				{
					...
					npc.broadcastNpcSay("Ah, estúpidos... Deus, eles...");
				}
				_elberoth = null;
				break;
			}
			case ARK_GUARDIAN_SHADOW_FANG:
			{
				if ((cond >= 5) && (cond <= 22) && !st.hasQuestItems(THIRD_KEY_OF_ARK))
				{
					...
					npc.broadcastNpcSay("Você não sabe... Sete selos são... tosses");
				}
				_shadowFang = null;
				break;
			}
