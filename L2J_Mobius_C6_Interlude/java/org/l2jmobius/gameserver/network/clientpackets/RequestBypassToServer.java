...
import org.l2jmobius.gameserver.ayengine.autofarm.FarmCommandHandler;
import org.l2jmobius.gameserver.ayengine.search.AySearchItem;
import org.l2jmobius.gameserver.ayengine.search.AySearchMob;
import org.l2jmobius.gameserver.ayengine.search.AySearchQuest;
	...
	public void run(GameClient client)
	{
		...
		try
		{
			if (_command.startsWith("admin_"))
			{
				...
				if (ach == null)
				{
					player.sendMessage("O comando " + command + " não existe!");
				...
				{
					player.sendMessage("Você não tem direito de acesso para usar este comando!");
					return;
				}
			...
			{
				FarmCommandHandler.getInstance().handleCommand(_command, player);
			}
			...
			else if (_command.startsWith("ayplayerinfo_"))
			{
				AyPlayerInfo.getInstance().useBypass(player, _command);
			}
			else if (_command.startsWith("aysearch_searchitem"))
			{
				AySearchItem.getInstance().useBypass(player, _command);
			}
			else if (_command.startsWith("aysearch_searchmob"))
			{
				AySearchMob.getInstance().useBypass(player, _command);
			}
			else if (_command.startsWith("aysearch_searchquest"))
			{
				AySearchQuest.getInstance().useBypass(player, _command);
			}
			else if (_command.startsWith("aysearch_searchdel"))
			{
				player.getRadar().removeAllMarkers();
				player.sendMessage("Marcas removidas do mapa!");
			}
			else if (_command.startsWith("donation"))
			{
				DonationManager.getInstance().handleBypass(player, _command.substring(9));
			}
		}
		catch (Exception e)
		{
