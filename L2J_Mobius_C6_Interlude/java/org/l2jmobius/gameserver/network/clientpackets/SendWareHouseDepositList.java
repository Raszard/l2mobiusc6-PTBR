...
public class SendWareHouseDepositList implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			player.sendMessage("Você está depositando itens muito rápido.");
			return;
		}
		
		if (player.getPrivateStoreType() != 0)
		{
			player.sendMessage("Você não pode depositar itens quando estiver negociando.");
			return;
		}
		...
		if (player.isDead())
		{
			player.sendMessage("Você não pode depositar itens enquanto estiver morto.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
		}
