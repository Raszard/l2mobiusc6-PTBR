...
public class RequestDropItem implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		{ // just head GM and admin can drop items on the ground
			player.sendPacket(SystemMessage.sendString("Você não tem o direito de descartar nada do inventário."));
			return;
		}
		...
		{
			player.sendPacket(SystemMessage.sendString("Você não pode descartar itens durante o encantamento."));
			return;
		}
		...
		{
			player.sendMessage("Você não pode largar itens, Configurado pelo GM!");
		...
		{
			player.sendMessage("Privilégios insuficientes.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		if (player.isCastingNow())
		{
			...
			if (skill != null)
			{
				...
				{
					player.sendMessage("Você não pode largar um item enquanto estiver lançando " + skill.getSkill().getName() + ".");
					return;
				}
			}
		}
