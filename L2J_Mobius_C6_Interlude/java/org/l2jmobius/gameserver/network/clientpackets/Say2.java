...
public class Say2 implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			player.sendMessage("Você não pode bater papo enquanto o banimento de bate-papo estiver em vigor.");
			return;
		}
		...
		{
			player.sendMessage("Você não pode conversar com jogadores fora da prisão.");
			return;
		}
		
		if (!client.getFloodProtectors().canPerformSayAction())
		{
			player.sendMessage("Não se pode falar rápido demais.");
			return;
		}
		
		if (player.isCursedWeaponEquiped() && ((chatType == ChatType.TRADE) || (chatType == ChatType.SHOUT)))
		{
			player.sendMessage("Gritar e trocar mensagens não podem ser usados ​​enquanto estiver em posse de uma arma amaldiçoada.");
			return;
		}
		...
		switch (chatType)
		{
			case WHISPER:
			{
				...
				if (!receiver.getBlockList().isInBlockList(player) || player.isGM())
				{
					...
					{
						player.sendMessage("O jogador está na prisão.");
						return;
					}
					if (receiver.isChatBanned() && !player.isGM())
					{
						player.sendMessage("O jogador foi banido do chat.");
						return;
					}
					if (receiver.isInOfflineMode())
					{
						player.sendMessage("O jogador está no modo offline.");
						return;
					}
			...
			case SHOUT:
			{
				...
				if (Config.DEFAULT_GLOBAL_CHAT.equalsIgnoreCase("on") || (Config.DEFAULT_GLOBAL_CHAT.equalsIgnoreCase("gm") && player.isGM()))
				{
					if (Config.GLOBAL_CHAT_WITH_PVP)
					{
						...
						{
							player.sendMessage("Você deve ter pelo menos " + Config.GLOBAL_PVP_AMOUNT + " mortes no pvp para falar no chat global");
							return;
						}
			...
			case TRADE:
			{
				if (Config.DEFAULT_TRADE_CHAT.equalsIgnoreCase("ON"))
				{
					if (Config.TRADE_CHAT_WITH_PVP)
					{
						...
						{
							player.sendMessage("Você deve ter pelo menos " + Config.TRADE_PVP_AMOUNT + " mortes no pvp para falar no chat de troca");
							return;
						}
						for (Player plr : World.getInstance().getAllPlayers())
						{
				...
				else if (Config.DEFAULT_TRADE_CHAT.equalsIgnoreCase("limited"))
				{
					...
					else if (Config.TRADE_CHAT_IS_NOOBLE)
					{
						...
						{
							player.sendMessage("Somente jogadores nobres podem usar este bate-papo");
							return;
						}
						final int region = MapRegionData.getInstance().getMapRegion(player.getX(), player.getY());
						for (Player plr : World.getInstance().getAllPlayers())
	...
	private void checkText(Player player)
	{
		if (Config.USE_SAY_FILTER)
		{
			...
			if (!filteredText.equalsIgnoreCase(_text))
			{
				...
				{
					player.setPunishLevel(IllegalActionPunishmentType.CHAT, Config.CHAT_FILTER_PUNISHMENT_PARAM1);
					player.sendMessage("O administrador baniu seu bate-papo de " + Config.CHAT_FILTER_PUNISHMENT_PARAM1 + " minutos");
				}
				else if (Config.CHAT_FILTER_PUNISHMENT.equalsIgnoreCase("karma"))
				{
					player.setKarma(Config.CHAT_FILTER_PUNISHMENT_PARAM2);
					player.sendMessage("Você recebeu " + Config.CHAT_FILTER_PUNISHMENT_PARAM2 + " karma por palavras ruins");
				}
				else if (Config.CHAT_FILTER_PUNISHMENT.equalsIgnoreCase("jail"))
				{
					player.setPunishLevel(IllegalActionPunishmentType.JAIL, Config.CHAT_FILTER_PUNISHMENT_PARAM1);
				}
				player.sendMessage("A palavra " + _text + " não é permitida!");
				_text = filteredText;
			}
		}
	}
