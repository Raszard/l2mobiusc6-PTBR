...
public class RequestActionUse implements ClientPacket
{
		...
		switch (_actionId)
		{
			...
			case 19: // pet unsummon
			{
				if ((pet != null) && !player.isBetrayed())
				{
					...
					{
						player.sendMessage("Você não pode fazer uma invocação desaparecer durante o combate."); // Message like L2OFF
					}
	...
	private void useSkill(GameClient client, int skillId, WorldObject target)
	{
		...
		{
			player.sendMessage("Não é possível usar habilidades durante a negociação");
			return;
		}
