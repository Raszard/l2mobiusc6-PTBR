...
public class RequestDuelStart implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (_partyDuel == 1)
		{
			...
			{
				player.sendMessage("Você precisa ser o líder de um grupo para solicitar um duelo de grupo.");
				return;
			}
			...
			{
				player.sendMessage("Este jogador é membro do seu próprio grupo.");
				return;
			}
			...
			for (Player temp : player.getParty().getPartyMembers())
			{
				if (!temp.canDuel())
				{
					player.sendMessage("Nem todos os membros do seu grupo estão prontos para um duelo.");
					return;
				}
			}
