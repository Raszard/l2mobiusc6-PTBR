...
public class RequestEnchantItem implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (player.getActiveTradeList() != null)
		{
			...
			player.sendMessage("Sua negociação foi cancelada.");
		...
		{
			player.sendMessage("Você não pode encantar itens quando tiver um armazém ativo ou comércio ativo.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		synchronized (item)
		{
			...
			else
			{
				if (crystalScroll)
				{
					sm = SystemMessage.sendString("Falhou no encantamento do cristal. O valor de encantamento do item se torna " + Config.CRYSTAL_ENCHANT_MIN);
					player.sendPacket(sm);
				}
