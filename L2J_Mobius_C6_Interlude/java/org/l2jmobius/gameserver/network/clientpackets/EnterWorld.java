...
public class EnterWorld implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			player.sendPacket(new CreatureSay(0, ChatType.CRITICAL_ANNOUNCE, "", "Evento de Final de Semana ativo: " + Config.WEEKEND_XP_MULTIPLIER + "x XP, " + Config.WEEKEND_SP_MULTIPLIER + "x SP, " + Config.WEEKEND_DROP_MULTIPLIER + "x Dops, " + Config.WEEKEND_SPOIL_MULTIPLIER + "x Spoils e " + Config.WEEKEND_DROP_MULTIPLIER + "x Adenas para todos os jogadores!"));
		}
		
		player.donatorMessage();
		
		if ((Config.SERVER_RESTART_SCHEDULE_ENABLED) && (Config.SERVER_RESTART_SCHEDULE_MESSAGE))
		{
			player.sendPacket(new CreatureSay(0, ChatType.WHISPER, "[Servidor]", "O próximo reinício está agendado para " + ServerRestartManager.getInstance().getNextRestartTime() + "."));
		}
		...
		if (!player.isGM() && (player.getSiegeState() < 2) && player.isInsideZone(ZoneId.SIEGE))
		{
			...
			player.teleToLocation(TeleportWhereType.TOWN);
			player.sendMessage("Você foi teletransportado para a cidade mais próxima por estar na zona de cerco");
		}
		...
		{
			player.sendMessage("Desculpe, mas multibox não é permitido aqui.");
			player.logout();
			return;
		}
	...
	private void Hellows(Player player)
	{
		...
		{
			player.sendMessage("Bem vindo ao " + Config.ALT_Server_Name);
		}
		
		if (Config.ONLINE_PLAYERS_ON_LOGIN)
		{
			player.sendMessage("Há " + World.getInstance().getAllPlayers().size() + " jogadores online.");
		}
		...
		{
			final String Welcome_Path = "data/html/CommunityBoard/top/index.htm";
			...
			if (mainText.exists())
			{
				...
				String content = HtmCache.getInstance().getHtm("data/html/CommunityBoard/top/index.htm");
	...
	private void notifyPartner(Player player)
	{
		if (player.getPartnerId() != 0)
		{
			...
			if (partner != null)
			{
				partner.sendMessage("Seu parceiro fez login");
			}
		}
	}
	...
	private void notifyCastleOwner(Player player)
	{
		...
		if ((clan != null) && (clan.getCastleId() > 0))
		{
			...
			{
				AnnouncementsTable.getInstance().announceToAll("Senhor " + player.getName() + " Governante do Castelo de " + castle.getName() + " está Online!");
			}
		}
	}
}