...
public class RequestSetPledgeCrest implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if ((_length < 0))
		{
			player.sendMessage("Erro de transferência de arquivo.");
			return;
		}
		
		if (_length > 256)
		{
			player.sendMessage("O tamanho do arquivo do brasão do clã era muito grande (máx. 256 bytes).");
			return;
		}
		...
		else
		{
			...
			if (crest != null)
			{
				...
				player.sendMessage("O brasão foi registrado com sucesso.");
			}
		}
	}
}
