...
public class Logout implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		{
			player.sendMessage("Você não pode sair enquanto estiver registrado em um evento.");
		...
		{
			player.sendMessage("Você não pode sair do jogo enquanto estiver no modo de combate.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			player.sendMessage("Você não pode sair do jogo enquanto estiver se teletransportando.");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			player.sendMessage("Você não pode sair do modo Olimpíada.");
			return;
		}
		...
		if (player.isFestivalParticipant())
		{
			...
			{
				player.sendMessage("Você não pode sair enquanto estiver participando de um festival.");
				return;
			}
			...
			if (playerParty != null)
			{
				player.getParty().broadcastToPartyMembers(SystemMessage.sendString(player.getName() + " foi removido do próximo festival."));
			}
		}
