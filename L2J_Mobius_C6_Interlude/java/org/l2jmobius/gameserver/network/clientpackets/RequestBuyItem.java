...
public class RequestBuyItem implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (_count == -1)
		{
			player.sendMessage("Você não pode comprar mais de 10.000 itens.");
			return;
		}
		
		if (!client.getFloodProtectors().canPerformTransaction())
		{
			player.sendMessage("ocê está comprando muito rápido.");
			return;
		}
		...
		{
			player.sendMessage("Alvo inválido: o vendedor deve ser o alvo");
			return;
		}
		...
		for (int i = 0; i < _count; i++)
		{
			...
			{
				player.sendMessage("Ohh Cheat não funciona? Você tem um problema agora!");
