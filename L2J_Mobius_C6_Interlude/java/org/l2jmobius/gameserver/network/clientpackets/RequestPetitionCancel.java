	...
	public void run(GameClient client)
	{
		...
		else if (PetitionManager.getInstance().isPlayerPetitionPending(player))
		{
			if (PetitionManager.getInstance().cancelActivePetition(player))
			{
				...
				final String msgContent = player.getName() + " cancelou uma petição pendente.";
				AdminData.broadcastToGMs(new CreatureSay(player.getObjectId(), ChatType.HERO_VOICE, "Sistema de Reclamações", msgContent));
			}
			else
			{
