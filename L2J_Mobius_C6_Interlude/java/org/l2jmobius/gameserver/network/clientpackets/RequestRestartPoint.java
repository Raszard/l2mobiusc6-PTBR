...
public class RequestRestartPoint implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if ((castle != null) && castle.getSiege().isInProgress() && (player.getClan() != null) && castle.getSiege().checkIsAttacker(player.getClan()))
		{
			...
			player.sendMessage("Você será renascido em " + (castle.getSiege().getAttackerRespawnDelay() / 1000) + " segundos");
			return;
		}
	...
	class DeathTask implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_player.sendMessage("Você não pode fazer isso enquanto participa de um evento!");
				return;
			}
				...
				switch (_requestedPointType)
				{
					case 1: // to clanhall
					{
						if (_player.getClan() != null)
						{
							...
							{
								// cheater
								_player.sendMessage("Você não pode usar este ponto de renascimento!");
