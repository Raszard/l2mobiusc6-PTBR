...
public class NpcTable
{
	...
	public NpcTemplate[] getTemplates()
	{
		return _npcs.values().toArray(new NpcTemplate[0]);
	}
	
	public List<NpcTemplate> getAllOfItemId(int itemID)
	{
		final List<NpcTemplate> list = new ArrayList<>();
		for (NpcTemplate t : _npcs.values())
		{
			for (DropData drops : t.getAllDropData())
			{
				if (drops.getItemId() == itemID)
				{
					list.add(t);
				}
			}
