...
public class SiegeManager
{
	...
	public boolean checkIfOkToSummon(Creature creature, boolean isCheckOnly)
	{
		...
		{
			message = "Você deve estar no terreno do castelo para invocar isto.";
		}
		else if (!castle.getSiege().isInProgress())
		{
			message = "Você só pode invocar isso durante um cerco.";
		}
		else if ((player.getClanId() != 0) && (castle.getSiege().getAttackerClan(player.getClanId()) == null))
		{
			message = "Você só pode invocar isso como um atacante registrado.";
		}
		else
		{
			return true;
