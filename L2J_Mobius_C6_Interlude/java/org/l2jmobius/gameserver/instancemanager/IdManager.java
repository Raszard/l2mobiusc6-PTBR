...
public class IdManager
{
	...
	public IdManager()
	{
			...
			Statement statement = con.createStatement())
		{
			...
			cleanCount += statement.executeUpdate("DELETE FROM cursed_weapons WHERE cursed_weapons.playerId NOT IN (SELECT charId FROM characters);");
		...
		try
		{
				...
				Statement statement = con.createStatement())
			{
				...
				try (ResultSet result = statement.executeQuery(extractUsedObjectIdsQuery.toString()))
				{
					...
					try (ResultSet rs = statement.executeQuery("SELECT purchase_id FROM donations"))
					{
						while (rs.next())
						{
