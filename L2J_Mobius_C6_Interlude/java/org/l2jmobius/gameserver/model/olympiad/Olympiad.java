...
public class Olympiad
{
	...
	private static final String OLYMPIAD_GET_HEROS = "SELECT olympiad_nobles.charId, characters.char_name FROM olympiad_nobles, characters WHERE characters.charId = olympiad_nobles.charId AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= 9 AND olympiad_nobles.competitions_won >= 1 ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.competitions_won/olympiad_nobles.competitions_done DESC LIMIT " + Config.ALT_OLY_NUMBER_HEROS_EACH_CLASS;
	...
	private void load()
	{
		...
		if (!loaded)
		{
			...
			try
			{
				final File file = new File(OLYMPIAD_DATA_FILE);
				if (file.exists())
				{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			while (rset.next())
			{
				...
				statData.set(COMP_DRAWN, rset.getInt(COMP_DRAWN));
				statData.set("to_save", false);
				NOBLES_RANK.put(charId, statData);
			}
	...
	protected class OlympiadEndTask implements Runnable
	{
		...
		public void run()
		{
			...
			AnnouncementsTable.getInstance().announceToAll("O período de validação das Olimpíadas começou");
	...
	protected class ValidationEndTask implements Runnable
	{
		...
		public void run()
		{
			AnnouncementsTable.getInstance().announceToAll("O período de validação das Olimpíadas terminou");
			_period = 0;
			_currentCycle++;
			deleteNobles();
			setNewOlympiadEnd();
	...
	public boolean registerNoble(Player noble, boolean classBased)
	{
		...
		{
			noble.sendMessage("Você não pode participar da Olimpíada. Você está no Modo Observador, tente reiniciar!");
			return false;
		}
		...
		if ((noble._activeBoxes > 1) && !Config.ALLOW_DUALBOX_OLY)
		{
			...
			if ((playerBoxes != null) && (playerBoxes.size() > 1))
			{
				for (String character_name : playerBoxes)
				{
					...
					{
						noble.sendMessage("Você já está participando da Olimpíada com outro personagem!");
						return false;
					}
				}
			}
		...
		{
			noble.sendMessage("Não é possível registrar quando você tem menos de 3 pontos");
			return false;
		}
		if (!classBased && (getNoblePoints(noble.getObjectId()) < 5))
		{
			noble.sendMessage("Não é possível registrar quando você tem menos de 5 pontos");
			return false;
		}
	...
	protected static synchronized void updateOldNobleStats(int playerId, StatSet stats)
	{
		...
		NOBLES_RANK.put(playerId, stats);
	...
	public boolean unRegisterNoble(Player noble)
	{
		...
		for (OlympiadGame game : OlympiadManager.getInstance().getOlympiadGames().values())
		{
			...
			{
				noble.sendMessage("Não é possível cancelar o registro enquanto você já foi selecionado para um jogo");
				return false;
			}
		}
	...
	public static void addSpectator(int id, Player spectator, boolean storeCoords)
	{
		...
		{
			spectator.sendMessage("Você já está registrado em um evento.");
			return;
		}
	...
	protected synchronized void saveOldNobleData(int nobleId)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			final StatSet nobleInfo = NOBLES_RANK.get(nobleId);
			if (nobleInfo == null)
			{
				return;
			}
	...
	{
		if (HEROS_TO_BE.isEmpty())
		{
			return;
		}
		
		for (StatSet hero : HEROS_TO_BE)
		{
			final int charId = hero.getInt(CHAR_ID);
			final StatSet noble = NOBLES.get(charId);
	...
	public int getNoblessePasses(int objId)
	{
		...
		if (NOBLES_RANK.isEmpty())
		{
			return 0;
		}
		
		final StatSet noble = NOBLES_RANK.get(objId);
		if (noble == null)
		{
			return 0;
		}
