...
class OlympiadGame
{
	...
	protected boolean portPlayersToArena()
	{
		...
		if (_playerOne.inObserverMode() || _playerTwo.inObserverMode())
		{
			...
			_playerOne.sendMessage("Um jogador nesta partida está no modo Observador! Partida abortada!");
			_playerTwo.sendMessage("Um jogador nesta partida está no modo Observador! Partida abortada!");
			_aborted = true;
			return false;
		}
	...
	protected void announceGame()
	{
		for (Spawn manager : Olympiad.OLY_MANAGERS)
		{
			if ((manager != null) && (manager.getLastSpawn() != null))
			{
				...
				manager.getLastSpawn().broadcastPacket(new CreatureSay(objId, ChatType.SHOUT, npcName, "A Olimpíada começará na Arena " + (_stadiumID + 1) + " em instantes."));
			}
		}
	}
...
class OlympiadGameTask implements Runnable
{
	...
	private boolean runGame()
	{
		...
		{
			_game._playerOne.sendMessage("Um jogador nesta partida está no modo Observador! Partida abortada!");
			_game._playerTwo.sendMessage("Um jogador nesta partida está no modo Observador! Partida abortada!");
			_game._aborted = true;
			return false;
		}
