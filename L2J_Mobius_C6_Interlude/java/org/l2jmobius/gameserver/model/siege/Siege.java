...
public class Siege
{
	...
	public class ScheduleEndSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					announceToPlayer((timeRemaining / 60000) + " minuto(s) até a conclusão do cerco de " + getCastle().getName() + ".", true);
				...
				{
					announceToPlayer("Cerco de " + getCastle().getName() + " tem " + (timeRemaining / 1000) + "segundo(s) restante(s)!", true);
	...
	public class ScheduleStartSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					announceToPlayer("O prazo de inscrição para cerco de " + getCastle().getName() + " terminou", false);
				...
				{
					announceToPlayer((timeRemaining / 60000) + " minuto(s) até o cerco de " + getCastle().getName() + " começar.", false);
				...
				{
					announceToPlayer("Cerco de " + getCastle().getName() + " faltam " + (timeRemaining / 1000) + " segundo(s) para começar!", false);
	...
	public void endSiege()
	{
		if (_isInProgress)
		{
			announceToPlayer("O cerco de " + getCastle().getName() + " terminou!", false);
			...
			{
				announceToPlayer("O cerco de " + getCastle().getName() + " terminou em empate.", false);
	...
	public void startSiege()
	{
		if (!_isInProgress)
		{
			...
			announceToPlayer("O cerco de " + getCastle().getName() + " começou!", false);
	...
	public void registerAttacker(Player player, boolean force)
	{
		...
		{
			player.sendMessage("Você não pode se registrar como um atacante porque sua aliança é dona do castelo");
			return;
		}
	...
	public void registerDefender(Player player, boolean force)
	{
		if (getCastle().getOwnerId() <= 0)
		{
			player.sendMessage("Você não pode se registrar como defensor porque de " + getCastle().getName() + " é propriedade do NPC.");
		}
	...
	private boolean checkIfCanRegister(Player player)
	{
		...
		{
			player.sendMessage("O prazo para inscrição no cerco de " + getCastle().getName() + " passou.");
		}
		...
		{
			player.sendMessage("Somente clãs com nível " + SiegeManager.getInstance().getSiegeClanMinLevel() + " ou superiores podem se registrar para um cerco ao castelo.");
		}
	...
	private void saveSiegeClan(Clan clan, int typeId, boolean isUpdateRegistration)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if ((typeId == 0) || (typeId == -1))
			{
				...
				announceToPlayer(clan.getName() + " foi registrado para defender " + getCastle().getName(), false);
			}
			else if (typeId == 1)
			{
				addAttacker(clan.getClanId());
				announceToPlayer(clan.getName() + " foi registrado para atacar " + getCastle().getName(), false);
			}
			else if (typeId == 2)
			{
				addDefenderWaiting(clan.getClanId());
				announceToPlayer(clan.getName() + " solicitou a defesa de " + getCastle().getName(), false);
			}
		}
		catch (Exception e)
		{
