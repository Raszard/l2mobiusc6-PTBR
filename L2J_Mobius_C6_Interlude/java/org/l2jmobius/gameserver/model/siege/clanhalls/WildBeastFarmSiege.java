...
public class WildBeastFarmSiege extends ClanHallSiege
{
	...
	public void startSiege()
	{
		...
		if ((_clansInfo.size() == 1) && (clanhall.getOwnerClan() != null))
		{
			...
			anonce("Acontece no cerco de seu quartel-general.", 1);
	...
	public void endSiege(boolean par)
	{
		...
		if (par)
		{
			...
			if (winner != null)
			{
				...
				anonce("Atenção! Salão do Clã, fazenda de bestas foi conquistado pelo clã " + winner.getName(), 2);
			}
			else
			{
				anonce("Atenção! Salão do Clã, fazenda de bestas roubada não tem dono", 2);
			}
		}
		setInProgress(false);
		unSpawnAll();
	...
	protected class startFirstStep implements Runnable
	{
		...
		public void run()
		{
			...
			anonce("A batalha começou. Mate o NPC inimigo", 1);
		}
	}
	...
	private final ExclusiveTask _startSiegeTask = new ExclusiveTask()
	{
		...
		protected void onElapsed()
		{
			...
			if ((registerTimeRemaining <= 0) && !_registrationPeriod)
			{
				...
				anonce("Atenção! O período de inscrição no salão do clã de cerco, fazenda de bestas.", 2);
	...
	public void anonce(String text, int type)
	{
		if (type == 1)
		{
			final CreatureSay cs = new CreatureSay(0, ChatType.SHOUT, "Jornal", text);
			for (String clanName : getRegisteredClans())
			{
				final Clan clan = ClanTable.getInstance().getClanByName(clanName);
