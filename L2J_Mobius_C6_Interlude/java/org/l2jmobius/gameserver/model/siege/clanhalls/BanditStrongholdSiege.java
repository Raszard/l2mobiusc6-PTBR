	...
	public void endSiege(boolean par)
	{
		...
		if (par)
		{
			...
			if (winner != null)
			{
				...
				anonce("Atenção! Salão do Clã, castelo foi conquistado pelo clã dos ladrões " + winner.getName(), 2);
			}
			else
			{
				anonce("Atenção! Salão do Clã, fortaleza roubada não tem dono", 2);
			}
		}
		setInProgress(false);
		unSpawnAll();
	...
	private final ExclusiveTask _startSiegeTask = new ExclusiveTask()
	{
		...
		protected void onElapsed()
		{
			...
			if ((registerTimeRemaining <= 0) && !_registrationPeriod)
			{
				...
				anonce("Atenção! O período de inscrição no cerco do Salão do Clã, ladrões de castelo.", 2);
