...
public class Castle
{
	...
	public void setOwner(Clan clan)
	{
		...
		if ((_ownerId > 0) && ((clan == null) || (clan.getClanId() != _ownerId)))
		{
			...
			if (oldOwner != null)
			{
				...
				AnnouncementsTable.getInstance().announceToAll(oldOwner.getName() + " perdeu o castelo de " + getName() + "!");
	...
	public void removeOwner(Clan clan)
	{
		if (clan != null)
		{
			...
			AnnouncementsTable.getInstance().announceToAll(clan.getName() + " perdeu o castelo de " + getName() + "!");
	...
	public void setTaxPercent(Player player, int taxPercent)
	{
		...
		{
			player.sendMessage("O valor do imposto deve estar entre 0 e " + maxTax + ".");
			return;
		}
		
		setTaxPercent(taxPercent);
		player.sendMessage("O imposto do castelo " + _name + " foi alterado para " + taxPercent + "%.");
	}
	...
	private void updateOwnerInDB(Clan clan)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if (clan != null)
			{
				...
				AnnouncementsTable.getInstance().announceToAll(clan.getName() + " tomou o castelo de " + getName() + "!");
				clan.broadcastToOnlineMembers(new PledgeShowInfoUpdate(clan));
