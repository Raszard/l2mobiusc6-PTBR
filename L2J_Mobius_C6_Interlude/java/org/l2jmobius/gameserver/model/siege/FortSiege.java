...
public class FortSiege
{
	...
	public class ScheduleStartSiegeTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				{
					announceToPlayer((timeRemaining / 60000) + " minuto(s) até o cerco de " + getFort().getName() + " começar.", false);
				...
				{
					announceToPlayer("O prazo de inscrição para " + getFort().getName() + " terminou.", false);
				...
				{
					announceToPlayer("Cerco de " + getFort().getName() + " começa em " + (timeRemaining / 1000) + " segundos!", false);
	...
	private List<Commander> _commanders = new ArrayList<>();
	private List<Artefact> _combatflag = new ArrayList<>();
	...
	public void endSiege()
	{
		if (_isInProgress)
		{
			announceToPlayer("O cerco de " + getFort().getName() + " acabou!", false);
			if (getFort().getOwnerId() <= 0)
			{
				announceToPlayer("O cerco de " + getFort().getName() + " terminou em empate.", false);
			}
	...
	public void startSiege()
	{
		if (!_isInProgress)
		{
			...
			announceToPlayer("O cerco de " + getFort().getName() + " começou!", false);
			saveFortSiege();
	 ...
	 * @param ct the ct
	 */
	public void killedCommander(Commander ct)
	{
		if (_commanders != null)
		{
	...
	public void registerAttacker(Player player, boolean force)
	{
		...
		{
			player.sendMessage("Você não pode se registrar como um atacante porque sua aliança é dona do forte");
			return;
		}
		
		if ((player.getInventory().getItemByItemId(57) != null) && (player.getInventory().getItemByItemId(57).getCount() < 250000))
		{
			player.sendMessage("Você não tem adena suficiente.");
			return;
		}
	...
	public void registerDefender(Player player, boolean force)
	{
		if (getFort().getOwnerId() <= 0)
		{
			player.sendMessage("Você não pode se registrar como defensor porque o forte " + getFort().getName() + " é propriedade do NPC.");
		}
	...
	private boolean checkIfCanRegister(Player player)
	{
		...
		{
			player.sendMessage("O prazo para inscrição no cerco de " + getFort().getName() + " passou.");
		}
		else if (_isInProgress)
		{
			player.sendMessage("Este não é o momento para registro de cerco e, portanto, o registro e o cancelamento não podem ser feitos.");
		}
		else if ((player.getClan() == null) || (player.getClan().getLevel() < FortSiegeManager.getInstance().getSiegeClanMinLevel()))
		{
			player.sendMessage("Somente clãs com nível " + FortSiegeManager.getInstance().getSiegeClanMinLevel() + " ou superiores podem se registrar para um cerco ao forte.");
		}
		else if (player.getClan().getFortId() > 0)
		{
			player.sendMessage("Você não pode se registrar porque seu clã já possui um forte.");
		}
		else if (player.getClan().getCastleId() > 0)
		{
			player.sendMessage("Você não pode se registrar porque seu clã já possui um castelo.");
		}
		...
		{
			player.sendMessage("Você já está registrado em um Siege.");
		}
		else
		{
			return true;
	...
	private void saveSiegeClan(Clan clan, int typeId, boolean isUpdateRegistration)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if ((typeId == 0) || (typeId == -1))
			{
				...
				announceToPlayer(clan.getName() + " foi registrado para defender " + getFort().getName(), false);
			}
			else if (typeId == 1)
			{
				addAttacker(clan.getClanId());
				announceToPlayer(clan.getName() + " foi registrado para atacar " + getFort().getName(), false);
			}
			else if (typeId == 2)
			{
				addDefenderWaiting(clan.getClanId());
				announceToPlayer(clan.getName() + " solicitou a defesa " + getFort().getName(), false);
			}
		}
		catch (Exception e)
		{
	...
	private void spawnCommander(int id)
	{
		...
		{
			Commander commander;
			commander = new Commander(IdManager.getInstance().getNextId(), NpcTable.getInstance().getTemplate(_sp.getNpcId()));
	...
	private void spawnFlag(int id)
	{
		...
		{
			Artefact combatflag;
			combatflag = new Artefact(IdManager.getInstance().getNextId(), NpcTable.getInstance().getTemplate(_sp.getNpcId()));
