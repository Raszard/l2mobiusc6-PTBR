...
public class Item extends WorldObject
{
	...
	public Item(int objectId, int itemId, Boolean isSkill)
	{
		super(objectId);
		...
		_itemId = itemId;
		_itemTemplate = ItemTable.getInstance().getTemplate(749);
		_count = 1;
		_loc = ItemLocation.VOID;
		_mana = _itemTemplate.getDuration();
	}
	 ...
	 */
	public void changeCountWithoutTrace(String process, int count, Player creator, WorldObject reference)
	{
		if (count == 0)
		{
			return;
	...
	public void onAction(Player player)
	{
		...
		if ((!player.isGM()) && (((_itemId >= 3960) && (_itemId <= 4021) && player.isInParty()) || ((_itemId >= 3960) && (_itemId <= 3969) && !player.isCastleLord(1)) || ((_itemId >= 3973) && (_itemId <= 3982) && !player.isCastleLord(2)) || ((_itemId >= 3986) && (_itemId <= 3995) && !player.isCastleLord(3)) || ((_itemId >= 3999) && (_itemId <= 4008) && !player.isCastleLord(4)) || ((_itemId >= 4012) && (_itemId <= 4021) && !player.isCastleLord(5)) || ((_itemId >= 5205) && (_itemId <= 5214) && !player.isCastleLord(6)) || ((_itemId >= 6779) && (_itemId <= 6788) && !player.isCastleLord(7)) || ((_itemId >= 7973) && (_itemId <= 7982) && !player.isCastleLord(8)) || ((_itemId >= 7918) && (_itemId <= 7927) && !player.isCastleLord(9))))
		{
			if (player.isInParty())
			{
				player.sendMessage("Você não pode pegar mercenários enquanto estiver em um grupo.");
			}
			else
			{
				player.sendMessage("Somente o senhor do castelo pode recrutar mercenários.");
			}
	...
	public void decreaseMana(boolean resetConsumingMana)
	{
		...
		final Player player = (Player) World.getInstance().findObject(getOwnerId());
		if (player != null)
		{
			SystemMessage sm;
			switch (_mana)
			...
			if (_mana == 0) // The life time has expired
			{
				...
				if (isEquipped())
				{
					...
					for (Item element : player.getInventory().unEquipItemInSlotAndRecord(getEquipSlot()))
					{
	 ...
	 * @return Item
	 */
	public static Item restoreFromDb(int objectId)
	{
		Item inst = null;
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if (rs.next())
			{
				...
				final ItemTemplate item = ItemTable.getInstance().getTemplate(item_id);
				if (item == null)
				{
	...
	{
		int Grade = ItemTable.getInstance().getTemplate(getItemId()).getCrystalType();
		switch (Grade)
		{
			case 1:
			{
