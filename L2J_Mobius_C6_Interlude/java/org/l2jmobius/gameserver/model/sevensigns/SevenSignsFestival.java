...
public class SevenSignsFestival implements SpawnListener
{
	...
	protected Map<Integer, List<Player>> _dawnFestivalParticipants;
	protected Map<Integer, List<Player>> _duskFestivalParticipants;
	protected Map<Integer, List<Player>> _dawnPreviousParticipants;
	protected Map<Integer, List<Player>> _duskPreviousParticipants;
	...
	protected void resetFestivalData(boolean updateSettings)
	{
		...
		for (Player onlinePlayer : World.getInstance().getAllPlayers())
		{
			try
			{
				final Item bloodOfferings = onlinePlayer.getInventory().getItemByItemId(FESTIVAL_OFFERING_ID);
	...
	public String getTimeToNextFestivalStart()
	{
		if (SevenSigns.getInstance().isSealValidationPeriod())
		{
			return "<font color=\"FF0000\">Este é o período de Validação do Selo. Os festivais serão retomados na semana que vem.</font>";
		}
		return "<font color=\"FF0000\">O próximo festival começará em " + getMinsToNextFestival() + " minuto(s).</font>";
	}
	...
	public int[] getFestivalForPlayer(Player player)
	{
		...
		{
			List<Player> participants = _dawnFestivalParticipants.get(festivalId);
	...
	public boolean isPlayerParticipant(Player player)
	{
		...
		for (List<Player> participants : _dawnFestivalParticipants.values())
		{
		...
		for (List<Player> participants : _duskFestivalParticipants.values())
		{
			if (participants.contains(player))
			{
				return true;
	...
	public boolean setFinalScore(Player player, int oracle, int festivalId, int offeringScore)
	{
		...
		if (offeringScore > thisCabalHighScore)
		{
			...
			final List<Player> prevParticipants = getPreviousParticipants(oracle, festivalId);
	...
	private class FestivalManager implements Runnable
	{
		...
		public synchronized void run()
		{
			...
			sendMessageToAll("Guia do Festival", "O evento principal começará em " + getMinsToNextFestival() + " minutos. Por favor, registre-se agora.");
			...
			sendMessageToAll("Guia do Festival", "O evento principal está começando agora.");
			...
			for (DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("O festival está prestes a começar!");
			}
			...
			for (DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("O festival terminará em " + ((Config.ALT_FESTIVAL_LENGTH - Config.ALT_FESTIVAL_SECOND_SPAWN) / 60000) + " minuto(s).");
			}
			...
			for (DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("Os baús surgiram! Seja rápido, o festival vai acabar logo.");
			}
			...
			sendMessageToAll("Bruxa do Festival", "Isso vai servir! Vou te levar para fora em breve.");
		}
	...
	private class DarknessFestival
	{
		...
		private final List<FestivalMonster> _npcInsts;
		private List<Player> _participants;
		private final Map<Player, FestivalSpawn> _originalLocations;
		...
		protected void festivalInit()
		{
			...
			if (!_participants.isEmpty())
			{
				try
				{
					for (Player participant : _participants)
					{
			...
			sendMessageToParticipants("O festival começará em 2 minutos.");
		}
			...
			if (!_participants.isEmpty())
			{
				for (Player participant : _participants)
				{
					try
					{
						...
						participant.sendMessage("O festival acabou. O líder do seu grupo deve agora registrar sua pontuação antes que o próximo festival aconteça.");
					}
					catch (NullPointerException e)
					{
					}
		...
		public void relocatePlayer(Player participant, boolean isRemoving)
		{
			try
			{
				...
				participant.sendMessage("Você foi removido da arena do festival.");
			}
			catch (Exception e)
			{
