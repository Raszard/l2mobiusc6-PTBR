...
public class EffectSilentMove extends Effect
{
	...
	public void onStart()
	{
		...
		if ((getSkill().getId() == 411) && (effected instanceof Player))
		{
			setAttackSilent(true);
			((Player) effected).getAppearance().setInvisible();
			((Player) effected).decayMe();
			((Player) effected).broadcastUserInfo();
			((Player) effected).spawnMe();
		}
	...
	public void onExit()
	{
		...
		if ((getSkill().getId() == 411) && (effected instanceof Player))
		{
			...
			((Player) effected).getAppearance().setVisible();
			((Player) effected).broadcastUserInfo();
		}
		else if (effected instanceof Player)
		{
	...
	private void setAttackSilent(boolean value)
	{
		if (getEffected() instanceof Player)
		{
			((Player) getEffected()).setAttackSilent(value);
		}
	}
}