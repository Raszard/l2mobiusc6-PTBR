...
public abstract class Inventory extends ItemContainer
{
	...
	private final Item[] _skinsPaperdoll;
	...
	{
		Item item;
		if (_skinsPaperdoll[slot] == null)
		{
			item = _paperdoll[slot];
		}
		else
		{
			item = _skinsPaperdoll[slot];
		}
		if (item != null)
		{
	...
	public void setSkinsPaperdoll(int slot, Item item)
	{
		_skinsPaperdoll[slot] = item;
	}
	
	public Item[] getSkinsPaperdoll()
	{
		return _skinsPaperdoll;
	}
	
	public Item getSkinPaperdollItem(int slot)
	{
		return _skinsPaperdoll[slot];
	}
	...
	private void unEquipItemInBodySlot(int slot)
	{
		...
		switch (slot)
		{
			...
			case ItemTemplate.SLOT_FULL_ARMOR:
			{
				...
				if ((chest != null) && ((chest.getTemplate().getItemId() == 9302) || (chest.getTemplate().getItemId() == 9303)))
				{
					if (_skinsPaperdoll[PAPERDOLL_CHEST] == null)
					{
						setSkinsPaperdoll(PAPERDOLL_DHAIR, null);
						setSkinsPaperdoll(PAPERDOLL_HAIR, null);
						setSkinsPaperdoll(PAPERDOLL_FACE, null);
					}
				}
				pdollSlot = PAPERDOLL_CHEST;
				break;
	...
	public synchronized void equipItem(Item item)
	{
		...
		switch (targetSlot)
		{
			...
			case ItemTemplate.SLOT_FULL_ARMOR:
			{
				if ((item.getItemId() != 9302) && (item.getItemId() != 9303))
				{
					final Item chest = getPaperdollItem(PAPERDOLL_CHEST);
					if ((chest != null) && ((chest.getTemplate().getItemId() == 9302) || (chest.getTemplate().getItemId() == 9303)))
					{
				...
				else
				{
					...
					if (_skinsPaperdoll[PAPERDOLL_CHEST] == null)
					{
						setSkinsPaperdoll(PAPERDOLL_DHAIR, placeholder);
						setSkinsPaperdoll(PAPERDOLL_HAIR, placeholder);
						setSkinsPaperdoll(PAPERDOLL_FACE, placeholder);
					}
					setPaperdollItem(PAPERDOLL_CHEST, null);
					setPaperdollItem(PAPERDOLL_LEGS, null);
					setPaperdollItem(PAPERDOLL_CHEST, item);
	...
	public void restore()
	{
		...
		{
			final Player player = (Player) getOwner();
			SkinTable.getInstance().restore(this, player.getObjectId());
		}
	}
