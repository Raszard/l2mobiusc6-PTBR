...
public class VillageMaster extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			{
				player.sendMessage("Você não pode mudar de subclasse quando estiver em combate.");
				return;
			}
			
			if (player.isCursedWeaponEquiped())
			{
				player.sendMessage("Você não pode mudar de subclasse enquanto a arma amaldiçoada estiver equipada!");
				return;
			}
			...
			switch (cmdChoice)
			{
				case 1: // Add Subclass - Initial
				{
					...
					{
						player.sendMessage("Agora você só pode alterar uma das suas subclasses atuais.");
						return;
					}
					subsAvailable = getAvailableSubClasses(player);
					if ((subsAvailable != null) && !subsAvailable.isEmpty())
					{
						content.append("Add Subclasse:<br>Qual subclasse você deseja adicionar?<br>");
					...
					{
						player.sendMessage("Não há subclasses disponíveis no momento.");
						return;
					}
					break;
				}
				...
				{
					content.append("Trocar Subclasse:<br>");
					final int baseClassId = player.getBaseClass();
					if (player.getSubClasses().isEmpty())
					{
						content.append("Você não pode alterar subclasses quando não tiver uma subclasse para começar.<br><a action=\"bypass -h npc_" + getObjectId() + "_Subclass 1\">Add subclasse.</a>");
					}
					else
					{
						content.append("Para qual classe você gostaria de mudar?<br>");
						if (baseClassId == player.getActiveClass())
						{
							content.append(PlayerTemplateData.getInstance().getClassNameById(baseClassId) + "&nbsp;<font color=\"LEVEL\">(Classe Base)</font><br><br>");
						}
						else
						{
							content.append("<a action=\"bypass -h npc_" + getObjectId() + "_Subclass 5 0\">" + PlayerTemplateData.getInstance().getClassNameById(baseClassId) + "</a>&nbsp;<font color=\"LEVEL\">(Classe Base)</font><br><br>");
						}
				...
				{
					content.append("Trocar Subclasse:<br>Qual das seguintes subclasses você gostaria de alterar?<br>");
					int classIndex = 1;
					for (Iterator<SubClass> subList = iterSubClasses(player); subList.hasNext();)
					{
						final SubClass subClass = subList.next();
						content.append("Sub-classe " + classIndex + "<br1>");
					...
					content.append("<br>Se você mudar de subclasse, você começará no nível 40 após a transferência da 2a classe.");
					break;
				}
				case 4: // Add Subclass - Action (Subclass 4 x[x])
				{
					...
					{
						LOGGER.warning(player + " realizou uma mudança de subclasse muito rápido");
					...
					{
						player.sendMessage("Você não pode adicionar uma subclasse durante um evento.");
						player.setLocked(false);
						return;
					}
					...
					{
						player.sendMessage("Você não pode adicionar uma nova subclasse antes de atingir o nível 75 da classe anterior.");
						allowAddition = false;
					}
					...
					if (allowAddition && !player.getSubClasses().isEmpty())
					{
						for (Iterator<SubClass> subList = iterSubClasses(player); subList.hasNext();)
						{
							...
							{
								player.sendMessage("Você não pode adicionar uma nova subclasse antes de atingir o nível 75 da subclasse anterior.");
								allowAddition = false;
								break;
							}
						}
					...
					if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS)
					{
						...
						{
							player.sendMessage("Você deve ter completado a missão Elixir de Mimir para continuar adicionando sua subclasse.");
							player.setLocked(false);
							return;
						}
						...
						if ((qs == null) || !qs.isCompleted())
						{
							player.sendMessage("Você deve ter concluído a missão Fate's Whisper para continuar adicionando sua subclasse.");
							player.setLocked(false);
							return;
						}
					}
					...
					if (allowAddition)
					{
						...
						{
							player.sendMessage("A subclasse não pôde ser adicionada.");
							player.setLocked(false);
							return;
						}
						...
						content.append("Add Subclasse:<br>A subclasse <font color=\"LEVEL\">" + className + "</font> foi adicionada.");
				...
				case 5: // Change Class - Action
				{
					...
					if (player.isRegisteredOnEvent())
					{
						player.sendMessage("Você não pode alterar a subclasse durante um evento.");
						player.setLocked(false);
						return;
					}
					...
					content.append("Trocar Subclasse:<br>Sua subclasse ativa agora é <font color=\"LEVEL\">" + PlayerTemplateData.getInstance().getClassNameById(player.getActiveClass()) + "</font>.");
				...
				{
					content.append("Escolha uma subclasse para a qual mudar. Se o que você procura não estiver aqui, procure o mestre adequado para sua classe.<br><font color=\"LEVEL\">Aviso!</font> Todas as classes e habilidades desta classe serão removidas.<br><br>");
					subsAvailable = getAvailableSubClasses(player);
					if ((subsAvailable != null) && !subsAvailable.isEmpty())
					{
						for (PlayerClass subClass : subsAvailable)
				...
				case 7: // Cancel/Change Subclass - Action
				{
					...
					if (player.isRegisteredOnEvent())
					{
						player.sendMessage("Você não pode excluir uma subclasse durante um evento.");
						player.setLocked(false);
						return;
					}
					...
					{
						player.setActiveClass(paramOne);
						content.append("Trocar Subclasse:<br>Sua subclasse foi alterada para <font color=\"LEVEL\">" + PlayerTemplateData.getInstance().getClassNameById(paramTwo) + "</font>.");
					...
					else
					{
						...
						player.sendMessage("A subclasse não pôde ser adicionada, você foi revertido para sua classe base.");
						player.setLocked(false);
						return;
					}
	 ...
	 */
	public void changeClanLeader(Player player, String target)
	{
		if (!player.isClanLeader())
		{
			player.sendPacket(SystemMessageId.YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT);
		...
		if (player.isFlying())
		{
			player.sendMessage("Saia do Wyvern primeiro.");
			return;
		}
		
		if (player.isMounted())
		{
			player.sendMessage("Retorne o Pet primeiro.");
			return;
		}
		...
		{
			player.sendMessage("Não é possível mudar o líder do clã enquanto estiver registrado no Siege");
			return;
		}
	...
	private void renameSubPledge(Player player, int pledgeType, String pledgeName)
	{
		...
		if (subPledge == null)
		{
			player.sendMessage("Juramento não existe.");
			return;
		}
		...
		player.sendMessage("Nome do juramento alterado.");
	}
	...
	private final Set<PlayerClass> getAvailableSubClasses(Player player)
	{
		...
		if (charClassId >= 88)
		{
			charClassId = ClassId.getClassId(charClassId).getParent().getId();
		}
	 ...
	 * @param player the player
	 */
	public void showPledgeSkillList(Player player)
	{
		...
		if (counts == 0)
		{
			...
			else
			{
				...
				sb.append("<html><body>");
				sb.append("Você aprendeu todas as habilidades disponíveis para o seu clã.<br>");
				sb.append("</body></html>");
	 ...
	 * @return the iterator
	 */
	private Iterator<SubClass> iterSubClasses(Player player)
	{
