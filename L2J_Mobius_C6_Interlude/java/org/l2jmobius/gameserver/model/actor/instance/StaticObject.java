...
{
	private static final Logger LOGGER = Logger.getLogger(StaticObject.class.getName());
	...
	public void onAction(Player player)
	{
		if (_type < 0)
		{
			LOGGER.info("StaticObject: StaticObject with invalid type! StaticObjectId: " + _staticObjectId);
		}
		...
		else
		{
			...
			if (!player.isInsideRadius2D(this, INTERACTION_DISTANCE))
			{
				...
				// Send a Server->Client packet ActionFailed (target is out of interaction range) to the Player player
			...
			else
			{
				if (_type == 2)
				{
					...
					if (content == null)
					{
						html.setHtml("<html><body>Falta a placa:<br>" + filename + "</body></html>");
					}
					else
					{
