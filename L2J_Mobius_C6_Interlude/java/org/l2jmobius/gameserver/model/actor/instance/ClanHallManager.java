 ...
 */
public class ClanHallManager extends Folk
{
	 ...
	 */
	public ClanHallManager(int objectId, NpcTemplate template)
	{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equalsIgnoreCase("manage_vault"))
			{
				...
				{
					player.sendMessage("Você não está autorizado a fazer isso!");
				}
				return;
			}
				...
				if ((player.getClanPrivileges() & Clan.CP_CH_SET_FUNCTIONS) == Clan.CP_CH_SET_FUNCTIONS)
				{
					if (val.equalsIgnoreCase("recovery"))
					{
						if (st.countTokens() >= 1)
						{
							...
							{
								player.sendMessage("Este clã Hall não tem dono, você não pode alterar a configuração");
								return;
							}
							val = st.nextToken();
							if (val.equalsIgnoreCase("hp"))
							{
								if (st.countTokens() >= 1)
								{
									...
									{
										player.sendMessage("Você não tem adena suficiente no armazém do seu clã");
									}
									else
									{
					...
					else if (val.equalsIgnoreCase("other"))
					{
						if (st.countTokens() >= 1)
						{
							if (getClanHall().getOwnerId() == 0)
							{
								player.sendMessage("Este Salão do Clã não tem dono, você não pode alterar a configuração");
								return;
							}
							val = st.nextToken();
			...
			else if (actualCommand.equalsIgnoreCase("support"))
			{
				...
				try
				{
					...
					{
						player.sendMessage("Nível de habilidade inválido!");
					}
				}
				catch (Exception e)
				{
	...
	public ClanHall getClanHall()
	{
		if (_clanHallId < 0)
		{
			final ClanHall clanHall = ClanHallTable.getInstance().getNearbyClanHall(getX(), getY(), getNpcId() == 35421 ? 1500 : 500); // Also check for devastated castle.
			if (clanHall != null)
			{
