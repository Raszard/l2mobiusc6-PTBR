 ...
 */
public class CastleChamberlain extends Folk
{
		...
		else if (condition == COND_OWNER)
		{
				...
				if (player.isClanLeader())
				{
					...
					html.replace("%castlename%", getCastle().getName());
					{
						...
						switch (currentPeriod)
						{
							...
							{
								html.replace("%ss_event%", "Inicialização do Evento de Missão");
								break;
							}
							case SevenSigns.PERIOD_COMPETITION:
							{
								html.replace("%ss_event%", "Competição (Evento de Missão)");
								break;
							}
							case SevenSigns.PERIOD_COMP_RESULTS:
							{
								html.replace("%ss_event%", "Resultados do Evento de Missão");
								break;
							}
							case SevenSigns.PERIOD_SEAL_VALIDATION:
							{
								html.replace("%ss_event%", "Validação do Selo");
								break;
							}
						}
					}
					{
						...
						switch (sealOwner1)
						{
							...
							{
								html.replace("%ss_avarice%", "Não Ocupado");
								break;
							}
							case SevenSigns.CABAL_DAWN:
							{
								html.replace("%ss_avarice%", "Sr. do Amanhecer");
								break;
							}
							case SevenSigns.CABAL_DUSK:
							{
								html.replace("%ss_avarice%", "Rv. do Crepúsculo");
								break;
							}
						}
					}
					{
						...
						switch (sealOwner2)
						{
							case SevenSigns.CABAL_NULL:
							{
								html.replace("%ss_gnosis%", "Não Ocupado");
								break;
							}
							case SevenSigns.CABAL_DAWN:
							{
								html.replace("%ss_gnosis%", "Sr. do Amanhecer");
								break;
							}
							case SevenSigns.CABAL_DUSK:
							{
								html.replace("%ss_gnosis%", "Rv. do Crepúsculo");
								break;
							}
						}
					}
					{
						...
						switch (sealOwner3)
						{
							case SevenSigns.CABAL_NULL:
							{
								html.replace("%ss_strife%", "Não Ocupado");
								break;
							}
							case SevenSigns.CABAL_DAWN:
							{
								html.replace("%ss_strife%", "Sr. do Amanhecer");
								break;
							}
							case SevenSigns.CABAL_DUSK:
							{
								html.replace("%ss_strife%", "Rv. do Crepúsculo");
								break;
							}
						}
					}
			...
			else if (actualCommand.equalsIgnoreCase("tax_set")) // tax rates control
			{
				if ((player.getClanPrivileges() & Clan.CP_CS_TAXES) == Clan.CP_CS_TAXES)
				{
					...
					msg.append("Taxa de imposto atual: " + getCastle().getTaxPercent() + "%<br>");
					msg.append("<table>");
					msg.append("<tr>");
					msg.append("<td>Alterar taxa de imposto para:</td>");
					msg.append("<td><edit var=\"value\" width=40><br>");
					msg.append("<button value=\"Ajustar\" action=\"bypass -h npc_%objectId%_tax_set $value\" width=80 height=15></td>");
					msg.append("</tr>");
					msg.append("</table>");
					msg.append("</center>");
