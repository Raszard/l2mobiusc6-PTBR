 ...
 * @author Tempy
 */
public class FestivalGuide extends Folk
{
		...
		else if (command.startsWith("Festival"))
		{
			...
			switch (val)
			{
				case 1: // Become a Participant
				{
					...
					{
						player.sendMessage("Você não pode se inscrever enquanto um festival estiver em andamento.");
						return;
					}
				...
				case 3: // Score Registration
				{
					...
					{
						player.sendMessage("Você não pode registrar uma pontuação enquanto um festival estiver em andamento.");
						return;
					}
					...
					{
						player.sendMessage("Você não tem nenhuma oferta de sangue para contribuir.");
						return;
					}
				...
				{
					final StringBuilder strBuffer = new StringBuilder("<html><body>Guia do Festival:<br>Estas são as melhores pontuações da semana, para o ");
					...
					if (dawnScore > 0)
					{
						strBuffer.append("Alvorecer: " + calculateDate(dawnData.getString("date")) + ". Pontuação " + dawnScore + "<br>" + dawnData.getString("members") + "<br>");
					}
					else
					{
						strBuffer.append("Alvorecer: Não existe nenhum registro. Pontuação 0<br>");
					}
					if (duskScore > 0)
					{
						strBuffer.append("Crepúsculo: " + calculateDate(duskData.getString("date")) + ". Pontuação " + duskScore + "<br>" + duskData.getString("members") + "<br>");
					}
					else
					{
						strBuffer.append("Crepúsculo: Não existe nenhum registro. Pontuação 0<br>");
					}
					if ((overallScore > 0) && (overallData != null))
					{
						...
						strBuffer.append("Melhores pontuações consecutivas: " + calculateDate(overallData.getString("date")) + ". Pontuação " + overallScore + "<br>Lado afiliado: " + cabalStr + "<br>" + overallData.getString("members") + "<br>");
					}
					else
					{
						strBuffer.append("Melhores pontuações consecutivas: Não existe registro. Pontuação 0<br>");
					}
				...
				case 0: // Distribute Accumulated Bonus
				{
					if (!SevenSigns.getInstance().isSealValidationPeriod())
					{
						player.sendMessage("Os bônus não podem ser pagos durante o período da competição.");
						return;
					}
