...
public class Folk extends Npc
{
	...
	public void showSkillList(Player player, ClassId classId)
	{
		...
		if (!getTemplate().canTeach(classId))
		{
			...
			final StringBuilder sb = new StringBuilder();
			sb.append("<html><body>");
			sb.append("Eu não posso te ensinar nenhuma habilidade.<br> Você deve encontrar os professores de sua classe atual.");
			sb.append("</body></html>");
			html.setHtml(sb.toString());
			player.sendPacket(html);
			return;
	 ...
	 * @param classId the class id
	 */
	public void showEnchantSkillList(Player player, ClassId classId)
	{
		final int npcId = getTemplate().getNpcId();
		if (_classesToTeach == null)
		{
		...
		if (player.getClassId().getId() < 88)
		{
			...
			final StringBuilder sb = new StringBuilder();
			sb.append("<html><body>");
			sb.append("Você deve ter a missão de mudança da 3ª classe concluída.");
			sb.append("</body></html>");
			html.setHtml(sb.toString());
			player.sendPacket(html);
			return;
		...
		if (counts == 0)
		{
			...
			else
			{
				final StringBuilder sb = new StringBuilder();
				sb.append("<html><body>");
				sb.append("Você aprendeu todas as habilidades para sua classe.<br>");
				sb.append("</body></html>");
				html.setHtml(sb.toString());
				player.sendPacket(html);
			}
		...
		if (command.startsWith("SkillList"))
		{
			if (Config.ALT_GAME_SKILL_LEARN)
			{
				...
				else
				{
					...
					String text = "<html><body><center>Aprendizagem de Habilidades:</center><br>";
					if (!ownClass)
					{
						final String mages = player.getClassId().isMage() ? "fighters" : "mages";
						text += "As habilidades de sua classe são as mais fáceis de se aprender.<br>Habilidades de outras classes são mais difíceis.<br>Habilidades para outra raça são ainda mais difíceis de aprender.<br>Você também pode aprender habilidades de " + mages + ", e eles são os mais difíceis de aprender!<br><br>";
					}
					...
					if (!_classesToTeach.isEmpty())
					{
						...
						while ((count == 0) && (classCheck != null))
						{
							for (ClassId cid : _classesToTeach)
							{
								...
								text += "<a action=\"bypass -h npc_%objectId%_SkillList " + cid.getId() + "\">Aprender " + cid + "'s Habilidade de Classe</a><br>\n";
								count++;
							}
