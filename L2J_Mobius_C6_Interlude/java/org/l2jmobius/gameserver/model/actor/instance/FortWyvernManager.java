...
public class FortWyvernManager extends Npc
{
	protected static final int COND_ALL_FALSE = 0;
	protected static final int COND_BUSY_BECAUSE_OF_SIEGE = 1;
	protected static final int COND_OWNER = 2;
	
	public FortWyvernManager(int objectId, NpcTemplate template)
	{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("RideWyvern"))
		{
			...
			{
				player.sendMessage("Somente líderes de clãs são permitidos.");
				return;
			}
			...
			{
				player.sendMessage("Monte no seu strider primeiro...");
			...
			if ((player.getInventory().getItemByItemId(1460) != null) && (player.getInventory().getItemByItemId(1460).getCount() >= 10))
			{
				...
				player.sendMessage("O Wyvern foi invocado com sucesso!");
			}
			else
			{
