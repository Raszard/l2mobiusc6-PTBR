...
public class Pet extends Summon
{
		...
		public void run()
		{
			try
			{
				...
				else
				{
					...
					stopFeed();
					unSummon(getOwner());
					getOwner().sendMessage("Seu animal de estimação está com muita fome para permanecer invocado.");
				}
	 ...
	 * @param control the control
	 */
	public Pet(int objectId, NpcTemplate template, Player owner, Item control)
	{
	...
	public void destroyControlItem(Player owner)
	{
		...
		try
		{
			final Item removedItem = owner.getInventory().destroyItem("PetDestroy", _controlItemId, 1, getOwner(), this);
	...
	private static Pet restore(Item control, NpcTemplate template, Player owner)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			{
				pet = new BabyPet(IdManager.getInstance().getNextId(), template, owner, control);
			}
			else
			{
				pet = new Pet(IdManager.getInstance().getNextId(), template, owner, control);
			}
