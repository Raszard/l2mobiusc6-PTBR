...
{
	protected static final Logger LOGGER = Logger.getLogger(Door.class.getName());
	 ...
	 */
	public Door(int objectId, CreatureTemplate template, int doorId, String name, boolean unlockable)
	{
	...
	public boolean isAutoAttackable(Creature attacker)
	{
		...
		else if (attacker instanceof Servitor)
		{
			player = ((Servitor) attacker).getOwner();
		}
	 ...
	 * <br>
	 * <b><u>Values</u>:</b><br>
	 * <li>object is a Player : 4000</li>
	 * <li>object is not a Player : 0</li><br>
	 * @param object the object
	...
	public void onAction(Player player)
	{
		...
		{
			if (!isInsideRadius2D(player, Npc.INTERACTION_DISTANCE))
			{
	...
	public void onActionShift(GameClient client)
	{
		...
		if (player.isGM())
		{
			...
			final StringBuilder html1 = new StringBuilder("<html><body><center><font color=\"LEVEL\">Informações da porta</font></center>");
			html1.append("<table border=0><tr>");
			html1.append("<td><button value=\"Matar\" action=\"bypass -h admin_kill\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\"></td>");
			html1.append("<td><button value=\"Deletar\" action=\"bypass -h admin_delete\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\"></td>");
			html1.append("<td><button value=\"Abrir\" action=\"bypass -h admin_open " + _doorId + "\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\"></td>");
			html1.append("<td><button value=\"Fechar\" action=\"bypass -h admin_close " + _doorId + "\" width=65 height=19 back=\"L2UI_ch3.smallbutton2_over\" fore=\"L2UI_ch3.smallbutton2\"></td>");
		...
		else
		{
			...
			final StringBuilder replyMsg = new StringBuilder("<html><body>Você não pode usar esta ação.");
	 ...
	 */
	public Collection<SiegeGuard> getKnownSiegeGuards()
	{
		final List<SiegeGuard> result = new ArrayList<>();
		for (WorldObject obj : getKnownList().getKnownObjects().values())
		{
			if (obj instanceof SiegeGuard)
			{
				result.add((SiegeGuard) obj);
			}
		}
		return result;
	}
	 ...
	 */
	public Collection<FortSiegeGuard> getKnownFortSiegeGuards()
	{
		final List<FortSiegeGuard> result = new ArrayList<>();
		final Collection<WorldObject> objs = getKnownList().getKnownObjects().values();
		{
			for (WorldObject obj : objs)
			{
				if (obj instanceof FortSiegeGuard)
				{
					result.add((FortSiegeGuard) obj);
				}
			}
		}
		return result;
	...
	{
		if (isAutoAttackable(attacker) || ((attacker instanceof Player) && ((Player) attacker).isGM()))
		{
	...
	public boolean doDie(Creature killer)
	{
		...
		if (isFort || isCastle)
		{
			broadcastPacket(SystemMessage.sendString("O portão do castelo foi quebrado."));
		}
		return true;
	}
