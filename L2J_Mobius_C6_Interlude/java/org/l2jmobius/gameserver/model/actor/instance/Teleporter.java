 ...
 */
public class Teleporter extends Folk
{
	 ...
	 */
	public Teleporter(int objectId, NpcTemplate template)
	{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		{
			player.sendMessage("Você não tem permissão para usar um teletransporte enquanto estiver registrado no jogo da olimpíada.");
			return;
		}
		
		if (player.isAio() && !Config.ALLOW_AIO_USE_GK)
		{
			player.sendMessage("Buffers Aio não podem usar teletransportes");
			return;
		}
		...
		if (actualCommand.equalsIgnoreCase("goto"))
		{
			...
			else if (condition == COND_OWNER)
			{
				...
				{
					player.sendMessage("Você não tem nível de acesso suficiente para se teletransportar para lá.");
				}
	...
	private void doTeleport(Player player, int value)
	{
		...
		if (list != null)
		{
			...
			{
				player.sendMessage("Não fuja do PvP! Você só poderá usar o teleportador depois que sua flag for embora.");
				return;
			}
			...
			{
				player.sendMessage("Você não pode usar o teletransporte quando estiver morto.");
				return;
			}
			else if (player.isSitting())
			{
				player.sendMessage("Você não pode usar o teletransporte enquanto estiver sentado.");
				return;
			}
			...
			else if (!list.isForNoble() && (Config.ALT_GAME_FREE_TELEPORT || player.destroyItemByItemId("Teleport", 9520, 1, this, true)))
			{
				player.teleToLocation(list.getX(), list.getY(), list.getZ(), true);
			}
