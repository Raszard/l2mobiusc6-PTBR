...
public class RaceManager extends Folk
{
	...
	public RaceManager(int objectId, NpcTemplate template)
	{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.equals("ShowOdds"))
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				// Odd
				final HistoryInfo odd = MonsterRace.getInstance().getHistory().get(MonsterRace.getInstance().getMonsters()[i].getTemplate().getNpcId() - 1);
				html.replace("Odd" + n, String.format(Locale.ENGLISH, "%.2f", odd.getOddRate()));
			}
			html.replace("1race", MonsterRace.getInstance().getRaceNumber());
			html.replace("%objectId%", getObjectId());
			player.sendPacket(html);
		...
		else if (command.equals("ShowTickets"))
		{
			...
			for (Item ticket : player.getInventory().getAllItemsByItemId(4443))
			{
				...
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h npc_%objectId%_ShowTicket ", ticket.getObjectId(), "\">", ticket.getEnchantLevel(), " Race Number</a></td><td align=right><font color=\"LEVEL\">", ticket.getCustomType1(), "</font> Número</td><td align=right><font color=\"LEVEL\">", ticket.getCustomType2() * 100, "</font> Adena</td></tr>");
			}
		...
		else if (command.equals("ViewHistory"))
		{
			...
			for (int i = history.size() - 1; i >= Math.max(0, history.size() - 7); i--)
			{
				...
				StringUtil.append(sb, "<tr><td><font color=\"LEVEL\">", info.getRaceId(), "</font> th</td><td><font color=\"LEVEL\">", info.getFirst(), "</font> Rota </td><td><font color=\"LEVEL\">", info.getSecond(), "</font> Lane</td><td align=right><font color=00ffff>", String.format(Locale.ENGLISH, "%.2f", info.getOddRate()), "</font> Tempo</td></tr>");
			}
