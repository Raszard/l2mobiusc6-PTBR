...
public class Auctioneer extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		{
			// TODO: html
			player.sendMessage("Condições erradas.");
			return;
		}
		...
		{
			// TODO: html
			player.sendMessage("Ocupado por causa do cerco.");
			return;
		}
		else if (condition == COND_REGULAR)
		{
			...
			if (actualCommand.equalsIgnoreCase("auction"))
			{
				...
				try
				{
					...
					{
						player.sendMessage("Lance inválido!");
					}
				}
				catch (Exception e)
				{
					player.sendMessage("Duração do leilão inválida!");
				}
				return;
			}
			if (actualCommand.equalsIgnoreCase("confirmAuction"))
			{
				...
				{
					player.sendMessage("Leilão inválido");
				}
				return;
			}
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				{
					player.sendMessage("O nível do seu clã precisa ser pelo menos 2, antes que você possa dar lances em um leilão");
					return;
				}
				...
				{
					player.sendMessage("Você não pode dar lances em mais de um leilão");
					return;
				}
			...
			else if (actualCommand.equalsIgnoreCase("doCancelBid"))
			{
				if (AuctionManager.getInstance().getAuction(player.getClan().getAuctionBiddedAt()) != null)
				{
					...
					player.sendMessage("Você cancelou com sucesso seu lance no leilão");
				}
				return;
			}
			else if (actualCommand.equalsIgnoreCase("cancelAuction"))
			{
				...
				{
					player.sendMessage("Você não tem os privilégios certos para fazer isso");
					return;
				}
