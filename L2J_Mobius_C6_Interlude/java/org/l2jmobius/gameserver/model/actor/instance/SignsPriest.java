...
public class SignsPriest extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			int stoneType = 0;
			final Item ancientAdena = player.getInventory().getItemByItemId(SevenSigns.ANCIENT_ADENA_ID);
			...
			switch (val)
			{
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
				{
					...
					{
						player.sendMessage("Você já é um membro do " + SevenSigns.getCabalName(cabal) + ".");
						return;
					}
					if (player.getClassId().level() == 0)
					{
						player.sendMessage("Você tem que ter concluído sua transferência de primeira classe.");
						break;
					}
					else if (player.getClassId().level() >= 2)
					{
						if (Config.ALT_GAME_REQUIRE_CASTLE_DAWN)
						{
							...
							{
								player.sendMessage("Você não precisa ser membro de um clã dono de castelo para se juntar aos Revolucionários do Crepúsculo.");
								return;
							}
							...
							if (!getPlayerAllyHasCastle(player))
							{
								...
								{
									player.sendMessage("Você deve ser membro de um clã dono de castelo para se juntar aos Senhores do Amanhecer.");
									return;
								}
							}
							else if (cabal == SevenSigns.CABAL_DAWN)
							{
								...
								if (!allowJoinDawn)
								{
									player.sendMessage("Você deve ser membro de um clã dono de castelo, ter um Certificado de Aprovação do Lorde ou pagar 50.000 adena para se juntar aos Senhores do Amanhecer.");
									return;
								}
							}
						}
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
				{
					...
					if (!stonesFound)
					{
						player.sendMessage("Você não tem nenhuma pedra de selo desse tipo.");
						break;
					}
				...
				case 18: // Exchange Seal Stones for Ancient Adena - SevenSigns 18 xxxx xxxxxx
				{
					...
					{
						player.sendMessage("Você deve inserir um valor inteiro.");
						break;
					}
					...
					{
						player.sendMessage("Você não tem tantas pedras de selo.");
					}
					break;
				}
				...
				{
					final StringBuilder contentBuffer = new StringBuilder("<html><body><font color=\"LEVEL\">[ Info. Selo ]</font><br>");
					for (int i = 1; i < 4; i++)
					{
						...
						{
							contentBuffer.append("[" + SevenSigns.getSealName(i, false) + ": Nada]<br>");
						}
					}
					contentBuffer.append("<a action=\"bypass -h npc_" + getObjectId() + "_SevenSigns 3 " + cabal + "\">Voltar.</a></body></html>");
					final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
	...
	private final boolean getPlayerAllyHasCastle(Player player)
	{
