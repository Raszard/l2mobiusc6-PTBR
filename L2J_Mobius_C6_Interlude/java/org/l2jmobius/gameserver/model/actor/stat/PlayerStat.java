...
import org.l2jmobius.gameserver.instancemanager.IndicationRewardManager;
...
public class PlayerStat extends PlayableStat
{
	...
	public void onLevelUp(Player player)
	{
		...
		{
			PreparedStatement ps = con.prepareStatement("SELECT indicated_id FROM indicated_players WHERE account_name=?");
			ps.setString(1, player.getAccountName());
			ResultSet rs = ps.executeQuery();
			
			if (rs.next())
			{
				int indicador = rs.getInt("indicated_id");
				int currentLevel = player.getLevel();
				
				Map<Integer, Integer> rewards = IndicationRewardManager.getRewards(currentLevel);
				if (rewards != null)
				{
					Player rewardPlayer = World.getInstance().getPlayer(indicador);
					if ((rewardPlayer != null))
					{
						for (Map.Entry<Integer, Integer> entry : rewards.entrySet())
						{
							...
							{
								rewardPlayer.addItem("indicated", itemId, itemCount, player, true);
								rewardPlayer.sendMessage(player.getName() + " alcançou o nível " + currentLevel + "! Você recebeu uma recompensa.");
							}
							else
							{
									...
									PreparedStatement ps2 = con.prepareStatement("INSERT INTO items VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"))
								{
									...
									ps.setInt(1, rewardPlayer.getInstanceId());
	...
	{
		if (getActiveChar().getFirstEffect(1000) != null)
		{
			return 1.5;
		}
		return 1.0;
