...
public class Player extends Playable
{
	...
	public class AIAccessor extends Creature.AIAccessor
	{
		...
		public void doAttack(Creature target)
		{
			...
			if (Config.ALLOW_CHAR_KILL_PROTECT && !isGM() && target.isPlayer() && (target.getActingPlayer().getPvpFlag() == 0) && (target.getActingPlayer().getKarma() == 0))
			{
				...
				{
					sendMessage("Você só pode participar de PvP quando seu alvo estiver dentro do seu nível.");
					sendPacket(ActionFailed.STATIC_PACKET);
					return;
				}
			}
	...
	public void checkIfWeaponIsAllowed()
	{
		...
		for (Effect currenteffect : getAllEffects())
		{
			...
			{
				sendMessage(effectSkill.getName() + " não pode ser usado com esta arma.");
				currenteffect.exit();
			}
		}
	}
	...
	public void giveAvailableSkills()
	{
		...
		sendMessage("Você aprendeu " + skillCounter + " novas habilidades.");
	}
	...
	public void destroyWearedItems(String process, WorldObject reference, boolean sendMessage)
	{
		...
		// Sends message to client if requested
		sendMessage("O modo de teste foi encerrado.");
	}
	...
	private static class TeleportProtectionFinalizer implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				{
					_player.sendMessage("O efeito de Proteção foi removido.");
				}
				else if (_player.isTeleportProtected())
				{
					_player.sendMessage("O efeito da Proteção de Teleporte foi removido.");
				}
	...
	public void doPickupItem(WorldObject object)
	{
		...
		synchronized (target)
		{
			...
			{
				sendMessage("Você atingiu a quantidade máxima de adena. Gaste ou deposite adena para poder continuar obtendo adena.");
				return;
			}
	...
	public void engageAnswer(int answer)
	{
		...
		if (ptarget != null)
		{
			if (answer == 1)
			{
				...
				ptarget.sendMessage("A solicitação de engajamento foi >ACEITA<");
			}
			else
			{
				ptarget.sendMessage("A solicitação de engajamento foi >NEGADO<!");
			}
		}
	}
	...
	public boolean doDie(Creature killer)
	{
		...
		{
			setHeroAura(false);
			sendMessage("Estado de Lenda de Guerra removido.");
		}
	...
	private boolean checkAntiFarm(Player targetPlayer)
	{
		if (Config.ANTI_FARM_ENABLED)
		{
			...
			{
				sendMessage("Farm é punível com Ban! GM informado.");
			...
			{
				sendMessage("Farm é punível com Ban! Não mate novos jogadores! GM informado.");
	...
	private void addItemReward(Player targetPlayer)
	{
		...
		if ((targetPlayer.getClient() != null) && (targetPlayer.getClient().getIp() != null))
		{
			if (targetPlayer.getClient().getIp() != _client.getIp())
			{
				if ((targetPlayer.getKarma() > 0) || (targetPlayer.getPvpFlag() > 0)) // killing target pk or in pvp
				{
					if (Config.PVP_REWARD_ENABLED)
					{
						...
						sendMessage("Você ganhou " + amount + " item(ns) de " + reward.getName() + ".");
					}
			...
			{
				sendMessage("Farm é punível com Ban! Não mate sua Box!");
	...
	public void increasePvpKills()
	{
		...
		if (Config.PVPEXPSP_SYSTEM)
		{
			...
			{
				sendMessage("Ganhei Exp e SP por uma morte pvp");
			}
		}
	...
	public boolean useMagic(Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (isSkillDisabled(skill))
		{
			...
			if ((skill.getId() == 2166 /* CP Gauge Potion */))
			{
				if (skill.getLevel() == 2)
				{
					sendMessage("A Poção de CP Maior não está disponível no momento: está sendo preparada para reutilização.");
				}
				else if (skill.getLevel() == 1)
				{
					sendMessage("A Poção CP não está disponível no momento: está sendo preparada para reutilização.");
				}
			}
			else
			{
	...
	public synchronized void setActiveClass(int classIndex)
	{
		...
		{
			sendMessage("Impossível mudar de classe se estiver em combate");
			sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
	...
	public void onPlayerEnter()
	{
		...
		if (SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod())
		{
			if (!isGM() && isIn7sDungeon() && (SevenSigns.getInstance().getPlayerCabal(this) != SevenSigns.getInstance().getCabalHighestScore()))
			{
				...
				sendMessage("Você foi teletransportado para a cidade mais próxima devido ao início do período de Validação do Selo.");
			}
		}
		else if (!isGM() && isIn7sDungeon() && (SevenSigns.getInstance().getPlayerCabal(this) == SevenSigns.CABAL_NULL))
		{
			teleToLocation(TeleportWhereType.TOWN);
			setIn7sDungeon(false);
			sendMessage("Você foi teletransportado para a cidade mais próxima porque não assinou com nenhuma selo.");
		}
		...
		DonationManager.getInstance().offlinePlayer(this);
	...
	public void onActionRequest()
	{
		if (isSpawnProtected())
		{
			...
			{
				sendMessage("Você não está mais protegido de monstros agressivos.");
			}
		}
		if (isTeleportProtected())
		{
			...
			if (!isInsideZone(ZoneId.PEACE))
			{
				sendMessage("A proteção teletransporte foi encerrada.");
			}
		}
	}
	...
	public void onTeleported()
	{
		...
		if ((Config.PLAYER_TELEPORT_PROTECTION > 0) && !isInOlympiadMode())
		{
			...
			sendMessage("Os efeitos da Proteção de Teletransporte fluem através de você.");
		}
	...
	public void startFishing(int x, int y, int z)
	{
		...
		if (_fish == null)
		{
			sendMessage("Erro - Peixes não estão definidos");
			endFishing(false);
			return;
		}
	...
	public void restoreCustomStatus()
	{
		...
		if (getAccountVariables().getLong("CustomDonatorEnd", 0) > currentTime)
		{
			setDonator(true);
		}
	...
	public void donatorMessage()
	{
		...
		if (getAccountVariables().getLong("CustomDonatorEnd", 0) > currentTime)
		{
			long donatorEndTime = getAccountVariables().getLong("CustomDonatorEnd");
			LocalDateTime endDateTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(donatorEndTime), java.time.ZoneId.systemDefault());
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
			String formattedDate = endDateTime.format(formatter);
			sendMessage("A doação terminará em: " + formattedDate);
		}
	}
			...
			case NONE: // Remove Punishments
			{
				switch (_punishLevel)
				{
					case CHAT:
					{
						...
						stopPunishTask(true);
						sendPacket(new EtcStatusUpdate(this));
						sendMessage("Sua proibição de bate-papo foi suspensa");
						break;
					}
					case JAIL:
					{
	...
	private void updatePunishState()
	{
		if (getPunishLevel() != IllegalActionPunishmentType.NONE)
		{
			...
			{
				_punishTask = ThreadPool.schedule(new PunishTask(this), _punishTimer);
				sendMessage("Você ainda está " + getPunishLevel().name().toLowerCase() + " por " + (_punishTimer / 60000) + " minutos.");
			}
