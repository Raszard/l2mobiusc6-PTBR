...
public class Npc extends Creature
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("npcfind_byid"))
		{
			...
			{
				player.sendMessage("Parâmetros de comando errados");
			}
		}
	...
	public void giveBlessingSupport(Player player)
	{
		...
		{
			final String content = "<html><body>Guia do Novato:<br>Sinto muito, mas você não é elegível para receber a bênção de proteção.<br1>Ela só pode ser concedida a <font color=\"LEVEL\">personagens abaixo do nível 39 que não fizeram transferência para segunda classe.</font></body></html>";
	...
	private void showQuestChooseWindow(Player player, List<Quest> quests)
	{
		...
		sb.append("<html><body><title>Falar sobre:</title><br>");
		String state;
		for (Quest q : quests)
		{
			if (q == null)
	...
	public void showQuestWindow(Player player, String questId)
	{
		...
		{
			// No quests found
			content = "<html><body>Você não está em uma missão que envolva este NPC ou não atende aos requisitos mínimos de missão deste NPC.</body></html>";
		}
		else
		{
			...
			if (qs == null)
			{
				if ((q.getQuestId() >= 1) && (q.getQuestId() < 20000))
				{
					...
					{
						player.sendMessage("Você tem muitas missões, não pode aceitar");
						return;
					}
				}
	...
	public void showLotoWindow(Player player, int value)
	{
		...
		else if ((value >= 1) && (value <= 21)) // 1-20 - buttons, 21 - second buy lottery ticket window
		{
			...
			if (count == 5)
			{
				final String search = "0\">Retornar";
				final String replace = "22\">O vencedor selecionou os números acima.";
				html.replace(search, replace);
			}
		}
		...
		else if (value == 24) // 24 - Previous winning numbers/Prize claim
		{
			...
			if (message == "")
			{
				message += "Não existe bilhete de loteria premiado...<br>";
			}
	...
	public void makeCPRecovery(Player player)
	{
		...
		if (player.isCursedWeaponEquiped())
		{
			player.sendMessage("Vá embora, você não é bem-vindo aqui.");
			return;
		}
	...
	public void makeSupportMagic(Player player)
	{
		...
		{
			final String content = "<html><body>Guia de Novatos:<br>Apenas para <font color=\"LEVEL\">personagem novato de nível " + higestLevel + " ou menor</font> pode receber minha magia de suporte.<br>Seu personagem novato é o primeiro que você criou neste mundo.</body></html>";
			insertObjectIdAndShowChatWindow(player, content);
			return;
		}
		...
		{
			final String content = "<html><body>Volte aqui quando atingir o nível " + lowestLevel + ". Eu vou te dar suporte mágico.</body></html>";
			insertObjectIdAndShowChatWindow(player, content);
			return;
		}
	...
	public void showChatWindow(Player player, int value)
	{
		...
		{
			html.replace("_Quest", "_RentPet\">Alugar Pet</a><br><a action=\"bypass -h npc_%objectId%_Quest");
		}
		html.replace("%npcname%", getName());
		html.replace("%playername%", player.getName());
		html.replace("%objectId%", String.valueOf(getObjectId()));
