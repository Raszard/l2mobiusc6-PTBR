...
public class ClanHallAuction
{
	...
	public synchronized void setBid(Player bidder, int bid)
	{
		...
		{
			bidder.sendMessage("O preço do lance deve ser maior");
		}
	}
	...
	private boolean takeItem(Player bidder, int quantity)
	{
		...
		bidder.sendMessage("Você não tem adena suficiente");
		return false;
	}
	...
	private void updateInDB(Player bidder, int bid)
	{
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			else
			{
				...
				{
					World.getInstance().getPlayer(_highestBidderName).sendMessage("Você foi superado em lance");
				}
			}
			...
			bidder.sendMessage("Você fez um lance com sucesso");
		}
		catch (Exception e)
		{
	...
	private void removeBids()
	{
		...
		for (Bidder b : _bidders.values())
		{
			...
			{
				World.getInstance().getPlayer(b.getName()).sendMessage("Parabéns, você ganhou o Salão do Clã!");
			}
