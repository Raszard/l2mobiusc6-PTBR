	...
	public void useItem(Playable playable, Item item)
	{
		...
		if (createItemId > 0)
		{
			if (ItemTable.getInstance().createDummyItem(createItemId) == null)
			{
				...
				player.sendMessage("Não aconteceu nada.");
				return;
			}
			
			if (ItemTable.getInstance().createDummyItem(createItemId).isStackable())
			{
				player.getInventory().addItem("Extract", createItemId, createAmount, player, null);
			...
			else
			{
				...
				{
					player.getInventory().addItem("Extract", createItemId, 1, player, null);
