...
public class ScrollOfEscape implements IItemHandler
{
	...
	public void useItem(Playable playable, Item item)
	{
		...
		if (player.isOnEvent())
		{
			player.sendMessage("Você não pode usar Pergaminho de Fuga em um evento.");
			return;
		}
		...
		{
			player.sendMessage("Você não pode usar SOE em PvP!");
			return;
		}
		...
		{
			player.sendPacket(SystemMessage.sendString("Você não pode usar uma habilidade de fuga em um festival."));
			return;
		}
		...
		if (player.isInJail())
		{
			player.sendPacket(SystemMessage.sendString("Você não pode escapar da prisão."));
			return;
		}
