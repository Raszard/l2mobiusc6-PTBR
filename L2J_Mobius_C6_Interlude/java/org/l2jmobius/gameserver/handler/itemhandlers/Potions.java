...
public class Potions implements IItemHandler
{
	...
	public synchronized void useItem(Playable playable, Item item)
	{
		if (playable instanceof Player)
		{
			...
			{
				activeChar.sendMessage("Você não pode usar poções em PvP!");
				return;
			}
			...
			switch (itemId)
			{
				...
				{
					if (!isEffectReplaceable(playable, EffectType.HEAL_OVER_TIME, itemId) || !isEffectReplaceable(playable, EffectType.MANA_HEAL_OVER_TIME, itemId))
					{
						return;
					}
				...
				case 9351: // lesser_mana_potion, xml: 9007
				{
					if (!isEffectReplaceable(playable, EffectType.MANA_HEAL_OVER_TIME, itemId))
					{
						return;
				...
				case 9352: // greater_mana_potion, xml: 9008
				{
					if (!isEffectReplaceable(playable, EffectType.MANA_HEAL_OVER_TIME, itemId))
					{
						return;
				...
				case 9353: // normal_mana_potion, xml: 9009
				{
					if (!isEffectReplaceable(playable, EffectType.MANA_HEAL_OVER_TIME, itemId))
					{
						return;
					}
					usePotion(activeChar, 9009, 1);
					break;
				}
			}
		}
		else if (playable instanceof Pet)
		{
			...
			switch (itemId)
			{
				...
				default:
				{
					activeChar.getOwner().sendMessage("Você não pode usar " + ItemTable.getInstance().getTemplate(itemId).getName() + " na invocação.");
				}
			}
		}
	}
				...
				if (skill != null)
				{
					...
					if (activeChar.isSkillDisabled(skill))
					{
						...
						else if (skill.getId() == 2166)
						{
							if (skill.getLevel() == 2)
							{
								activeChar.sendMessage("A Poção de CP Maior não está disponível no momento: está sendo preparada para reutilização.");
							}
							else if (skill.getLevel() == 1)
							{
								activeChar.sendMessage("A Poção CP não está disponível no momento: está sendo preparada para reutilização.");
							}
						}
						return false;
					}
					...
					if ((magicId == 2031) || (magicId == 2032) || (magicId == 2037) || (magicId == 9007) || (magicId == 9008) || (magicId == 9009))
					{
	...
	enum PotionSkill
	{
		...
		lesser_mana_potion(9351, 9007, 1),
		greater_mana_potion(9352, 9008, 1),
		normal_mana_potion(9353, 9009, 1);
