...
public class SummonItems implements IItemHandler
{
	...
	public void useItem(Playable playable, Item item)
	{
		...
		{
			player.sendMessage("Você não pode usar isso enquanto estiver paralisado");
		...
		switch (sitem.getType())
		{
			case 0: // static summons (like christmas tree)
			{
				try
				{
					...
					player.sendMessage("Criado " + npcTemplate.getName() + " em x: " + spawn.getX() + " y: " + spawn.getY() + " z: " + spawn.getZ());
				}
				catch (Exception e)
				{
					player.sendMessage("O alvo não está no jogo.");
				}
				break;
			}
			case 1: // pet summons
