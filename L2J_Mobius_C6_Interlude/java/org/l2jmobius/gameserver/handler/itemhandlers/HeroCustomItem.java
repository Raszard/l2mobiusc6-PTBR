...
public class HeroCustomItem implements IItemHandler
{
	...
	public void useItem(Playable playable, Item item)
	{
		if (Config.HERO_CUSTOM_ITEMS)
		{
			...
			{
				player.sendMessage("Este item não pode ser usado no modo olimpíada.");
			}
			
			if (player.isHero())
			{
				player.sendMessage("Você já é um herói!");
			}
			else
			{
				player.broadcastPacket(new SocialAction(player.getObjectId(), 16));
				player.setHero(true);
				player.sendMessage("Agora você é um herói, recebeu status de herói, habilidades e aura.");
