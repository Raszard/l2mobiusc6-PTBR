...
public class StrSiegeAssault implements ISkillHandler
{
	...
	public void useSkill(Creature creature, Skill skill, List<Creature> targets)
	{
		...
		try
		{
			...
			for (WorldObject target2 : targets)
			{
				...
				{
					creature.sendPacket(SystemMessage.sendString(skill.getName() + " falhou."));
				}
			}
		}
		catch (Exception e)
		{
			player.sendMessage("Erro ao usar ataque de cerco:" + e);
		}
	}
	...
	public static boolean checkIfOkToUseStriderSiegeAssault(Creature creature, Castle castle, boolean isCheckOnly)
	{
		...
		{
			message = "Você deve estar no terreno do castelo para usar o ataque de cerco do strider";
		}
		else if (!castle.getSiege().isInProgress())
		{
			message = "Você só pode usar o ataque de cerco do strider durante um cerco.";
		}
		else if (!(player.getTarget() instanceof Door))
		{
			message = "Você só pode usar o ataque de cerco do strider em portas e paredes.";
		}
		else if (!creature.isRiding())
		{
			message = "Você só pode usar o ataque de cerco do strider quando estiver no strider.";
		}
		else
		{
			return true;
	...
	public static boolean checkIfOkToUseStriderSiegeAssault(Creature creature, Fort fort, boolean isCheckOnly)
	{
		...
		{
			message = "Você deve estar no terreno do forte para usar o ataque de cerco do strider.";
		}
