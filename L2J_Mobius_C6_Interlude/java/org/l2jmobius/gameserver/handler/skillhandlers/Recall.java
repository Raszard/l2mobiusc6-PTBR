	...
	public void useSkill(Creature creature, Skill skill, List<Creature> targets)
	{
		try
		{
			if (creature instanceof Player)
			{
				...
				{
					((Player) creature).sendMessage("Você não pode invocar na zona de cerco.");
					return;
				}
			...
			for (Creature target : targets)
			{
				if (target instanceof Player)
				{
					...
					{
						targetChar.sendPacket(SystemMessage.sendString("Você não pode usar a habilidade de fuga em um festival."));
						continue;
					}
					
					if (targetChar.isOnEvent())
					{
						targetChar.sendMessage("Você não pode usar a habilidade de fuga em um evento.");
						continue;
					}
					
					if (targetChar.isInJail())
					{
						targetChar.sendPacket(SystemMessage.sendString("Você não pode escapar da prisão."));
						continue;
					}
					
					if (targetChar.isInDuel())
					{
						targetChar.sendPacket(SystemMessage.sendString("Você não pode usar habilidades de fuga durante um duelo."));
						continue;
					}
