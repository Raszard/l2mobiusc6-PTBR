...
public class TakeCastle implements ISkillHandler
{
	...
	public static boolean checkIfOkToCastSealOfRule(Creature creature, Castle castle, boolean isCheckOnly)
	{
		...
		{
			message = "Você deve estar no terreno do castelo para usar esta habilidade.";
		}
		else if ((player.getTarget() == null) || !player.getTarget().isArtefact())
		{
			message = "Você só pode usar essa habilidade em um artefato.";
		}
		else if (!castle.getSiege().isInProgress())
		{
			message = "Você só pode usar essa habilidade durante um cerco.";
		}
		else if (!Util.checkIfInRange(200, player, player.getTarget(), true) || (Math.abs(player.getZ() - player.getTarget().getZ()) > 45))
		{
			message = "Você não está no alcance do artefato.";
		}
		else if (castle.getSiege().getAttackerClan(player.getClan()) == null)
		{
			message = "Você deve ser um atacante para usar esta habilidade.";
		}
		else
		{
			if (!isCheckOnly)
			{
				castle.getSiege().announceToPlayer("Clã " + player.getClan().getName() + " começou a cravar a bandeira.", true);
			}
			return true;
		}
	...
	public static boolean checkIfOkToCastFlagDisplay(Creature creature, Fort fort, boolean isCheckOnly)
	{
		...
		{
			message = "Você deve estar em terreno forte para usar esta habilidade.";
		}
		else if ((player.getTarget() == null) && !player.getTarget().isArtefact())
		{
			message = "Você só pode usar essa habilidade em um mastro de bandeira.";
		}
		...
		{
			message = "Você não está no alcance do mastro da bandeira.";
		}
		...
		else
		{
			if (!isCheckOnly)
			{
				fort.getSiege().announceToPlayer("Clã " + player.getClan().getName() + " começou a levantar bandeira.", true);
			}
			return true;
		}
